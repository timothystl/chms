...
- **Verified live 2026-07-28** (see `NOTES.md`): the connect → sync flow was exercised end-to-end against real production QuickBooks and completed successfully — this is not a doc-only claim. QuickBooks' own native `BudgetVsActuals` report endpoint is confirmed Intuit-side unsupported at this app's tier (ignores the requested date range); the app works around it via `mergeCurrentYearBudgetAndActual()`, which reconstructs Budget vs. Actual from the `Budget` entity plus a date-scoped `ProfitAndLoss` report instead. Before assuming the connection is still live: refresh tokens run ~100 days, and `finance_qb_connection` should be checked directly for whether a valid row still exists — if not, reconnecting is just "Connect QuickBooks" again in Settings, no code change needed.
