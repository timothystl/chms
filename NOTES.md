...
### 2026-04-23
- **v107**: WC1 + FU1 intake — public form ingestion from the church website. Two new server-to-server endpoints on this Worker: `POST /api/intake/connect-card` and `POST /api/intake/prayer`, both authed by `X-Intake-Key` against the new `CHMS_INTAKE_API_KEY` secret (wrangler secret put required before first use). Connect-card creates or merges a `people` row (email-first, phone-fallback lookup; only fills blank fields on merge — never clobbers curated data), seeds `first_contact_date=today` + `followup_status='new'` so the record lands in the FU2 queue, and appends the visitor's message to notes with a dated header. Prayer writes a row to the new `prayer_requests` table (status defaults `new`; `is_urgent` and `share_publicly` booleans supported; best-effort person link by email/phone; anonymous submissions allowed). Every payload — success or failure — is mirrored into the new `intake_submissions` log for forensic replay. Routes matched in `tlc-volunteer-worker.js` before the scheduler auth gate and before the `/api/*` Breeze catch-all. Handler is `src/api-intake.js`. New tables `prayer_requests` and `intake_submissions` added via `initDb()` CREATE-IF-NOT-EXISTS block.
  - Expected JSON body shapes (also documented in `src/api-intake.js`):
    - connect-card: `{ first_name, last_name, email, phone, address1?, city?, state?, zip?, notes?, interests?: string[], source? }` — returns `{ ok, person_id, merged, match }`.
    - prayer: `{ name, email?, phone?, request, is_urgent?, share_publicly?, source? }` — returns `{ ok, prayer_id, linked_person_id }`.
  - Deploy: `wrangler secret put CHMS_INTAKE_API_KEY`, then `wrangler deploy`. The public website Worker must be given the same secret value and updated to POST each form submission to the two URLs above.
