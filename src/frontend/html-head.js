...
<link href="https://fonts.googleapis.com/css2?family=Source+Sans+3:ital,wght@0,400;0,600;0,700;1,400&display=swap" rel="stylesheet" media="print" onload="this.media='all'">
<noscript><link href="https://fonts.googleapis.com/css2?family=Source+Sans+3:ital,wght@0,400;0,600;0,700;1,400&display=swap" rel="stylesheet"></noscript>
<style>
/* ── Timothy Workspace v1.0 "Open Sky" tokens ─────────────────────────────
   The design system (DESIGN-SYSTEM.md / tokens.json v1.0, 2026-09-24) that replaced the
   navy/teal/cream PAL1 palette. White work surfaces on a pale cloud page, slate text, ONE
   blue action color, soft-blue selection, Source Sans 3 throughout.
     Action:   --primary #386781 · --primary-hover #2B5065 · --tint #DFEBF2 (selected)
     Neutral:  --page #F3F7FA · --surface #FFFFFF · --text #293D49 · --muted #536B79
               · --border #D7E2E9 (decorative) · --control-border #718694 (inputs)
     Status:   success/warning/error/info, each with a pale -bg. They describe STATE only.
     Identity: --accent #C9973A (gold) — a small detail, never text, never focus.
   The legacy names further down (--color-navy, --warm-gray, --linen, --warm-*, ...) are
...
:root{
  ...
  --radius-control:8px;--radius-card:12px;--radius-panel:20px;
  --sidebar-width:224px;
  --font-ui:'Source Sans 3',system-ui,-apple-system,'Segoe UI',sans-serif;
  /* ── Connect brand: the four values declared by the logo sheet's palette row. The mark is
     artwork (icons/connect-mark.png); these are NOT UI colors. ── */
  --val-welcome:#5B994D;--val-receive:#1761AD;--val-grow:#3BA9B2;--val-go:#F0A413;
  /* ── Legacy names, aliased onto Open Sky ──
     navy was headings/ink and primary buttons; teal was the action/link color. */
  --color-navy:var(--brand-ink);--navy:var(--brand-ink);--steel-anchor:var(--brand-ink);
  --color-teal:var(--primary);--teal:var(--primary);
  --color-gold:var(--accent);--gold-accent:var(--accent);--amber:var(--accent);
  --color-cream:var(--page);--color-light-teal:var(--info-bg);
  --deep-steel:var(--primary-hover);--mid-steel:var(--primary);--sky-steel:var(--primary);
  --ice-blue:var(--tint);--blue-mist:var(--info-bg);
  --deep-amber:var(--warning);--pale-gold:var(--warning-bg);
  --sage:var(--success);--pale-sage:var(--success-bg);
  --warm-white:var(--page);--linen:var(--page);--white:#FFFFFF;
  --charcoal:var(--text);--warm-gray:var(--muted);
  /* P26-A / DSN1: tokens the embedded Scheduler uses but whose own :root is stripped on
     embed (see test/scheduler-css-vars.test.js). */
  --honey:#E8C070;--soft-sage:#9AB89E;--on-pale-gold:var(--warning);--on-pale-sage:var(--success);
  --on-error-bg:var(--error);--error-border:var(--error);
  --danger-btn:var(--error);--danger-hover:#83221D;
  --font-display:var(--font-ui);--font-head:var(--font-ui);--font-body:var(--font-ui);
  --danger:var(--error);
  --bg:var(--page);--faint:var(--control-border);
  /* ── Former "warm redesign" tokens (People list / Person Profile / Household View) ── */
  --warm-ink-label:var(--text);--warm-meta:var(--muted);
  --warm-border:var(--border);--warm-divider:var(--border);--warm-row-divider:var(--border);
  --warm-surface-card:var(--surface);--warm-surface-page:var(--page);
  --warm-surface-header:var(--page);--warm-surface-card-page:var(--page);
  --status-member:var(--success);--status-visitor:var(--primary);--status-associate:var(--primary);
  --status-friend:var(--muted);--status-inactive:var(--warning);--status-organization:var(--muted);
  /* ── Volunteer/Events handoff tokens (P26-C). --ev-navy/--ev-teal/--ev-ink/--ev-danger are
     retired (test/ev-token-retirement.test.js); the five below have no one-word brand name. ── */
  --ev-muted:var(--muted);
  --ev-border:var(--border);--ev-border2:var(--control-border);
  --ev-cream:var(--page);--ev-moss:var(--success);
}
...
body{font-family:var(--font-body);font-size:16px;line-height:1.5;background:var(--page);color:var(--text);-webkit-font-smoothing:antialiased;}
...
.sidebar{position:fixed;left:0;top:0;height:100vh;height:100dvh;width:var(--sidebar-width);background:var(--surface);border-right:1px solid var(--border);display:flex;flex-direction:column;align-items:stretch;padding:24px 12px 16px;gap:2px;overflow-y:auto;transition:left .15s ease;z-index:200;}
.sidebar.open{left:0;}
...
.s-word-rule{display:none;}
.s-org{font-size:12px;font-weight:600;color:var(--muted);text-align:center;line-height:1.3;}
.s-item{width:100%;min-height:44px;border-radius:var(--radius-control);display:flex;align-items:center;justify-content:flex-start;padding:0 12px;gap:12px;cursor:pointer;position:relative;flex-shrink:0;transition:background-color .15s ease,color .15s ease;overflow:hidden;white-space:nowrap;color:var(--text);}
.s-item:hover{background:var(--page);}
.s-item.active{background:var(--tint);color:var(--primary);box-shadow:inset 3px 0 0 var(--primary);font-weight:600;}
.s-item:focus-visible{outline:3px solid var(--focus);outline-offset:-3px;}
.s-item svg{width:20px;height:20px;fill:none;stroke:var(--muted);stroke-width:2;stroke-linecap:round;stroke-linejoin:round;flex-shrink:0;}
.s-item.active svg{stroke:var(--primary);}
.s-divider{width:100%;height:1px;background:var(--border);margin:8px 0;flex-shrink:0;}
.s-section-hdr{font-family:var(--font-body);font-size:12px;font-weight:700;letter-spacing:.12em;text-transform:uppercase;color:var(--muted);padding:16px 12px 4px;white-space:nowrap;}
.s-bottom{margin-top:auto;display:flex;flex-direction:column;align-items:stretch;gap:2px;padding-top:12px;border-top:1px solid var(--border);}
...
.s-version{padding:8px 12px 0;font-size:12px;color:var(--muted);white-space:nowrap;}
.s-tip{position:static;transform:none;background:transparent;border:none;padding:0;font-size:16px;color:inherit;white-space:nowrap;pointer-events:none;z-index:auto;}
/* ── CONTENT AREA ── */
.content-area{flex:1;display:flex;flex-direction:column;overflow:hidden;margin-left:var(--sidebar-width);min-width:0;}
/* ── TOPBAR ── */
.topbar{min-height:64px;border-bottom:1px solid var(--border);display:flex;align-items:center;padding:0 32px;gap:12px;flex-shrink:0;background:var(--page);}
...
.topbar-title{font-size:24px;font-weight:700;color:var(--brand-ink);letter-spacing:-.01em;flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
...
.topbar-role{display:none;font-size:14px;padding:4px 10px;border-radius:var(--radius-control);background:var(--tint);color:var(--primary);font-weight:600;}
...
.search-wrap input{width:100%;min-height:40px;padding:8px 12px 8px 36px;border:1px solid var(--control-border);border-radius:var(--radius-control);font-family:var(--font-body);font-size:15px;color:var(--text);background:var(--surface);}
...
.pill.active{background:var(--tint);color:var(--primary);border-color:var(--primary);}
...
/* ── BUTTONS ── */
.btn-primary{min-height:40px;padding:8px 18px;background:var(--primary);color:#FFFFFF;border:1px solid var(--primary);border-radius:var(--radius-control);font-family:var(--font-body);font-size:15px;font-weight:600;cursor:pointer;transition:background-color .15s ease;display:inline-flex;align-items:center;justify-content:center;gap:6px;}
.btn-primary:hover{background:var(--primary-hover);border-color:var(--primary-hover);}
.btn-secondary{min-height:40px;padding:8px 16px;background:var(--surface);color:var(--primary);border:1px solid var(--control-border);border-radius:var(--radius-control);font-family:var(--font-body);font-size:15px;font-weight:600;cursor:pointer;transition:background-color .15s ease;display:inline-flex;align-items:center;justify-content:center;gap:6px;}
.btn-secondary:hover{background:var(--tint);}
.btn-danger{min-height:40px;padding:8px 16px;background:var(--surface);color:var(--error);border:1px solid var(--error);border-radius:var(--radius-control);font-family:var(--font-body);font-size:15px;font-weight:600;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;gap:6px;}
.btn-danger:hover{background:var(--error-bg);}
.btn-primary:active,.btn-secondary:active,.btn-danger:active,.btn-sm:active{transform:translateY(1px);}
.btn-primary:disabled,.btn-secondary:disabled,.btn-danger:disabled,.btn-sm:disabled{opacity:.5;cursor:not-allowed;transform:none;}
...
.p-card{background:var(--white);border:1px solid var(--border);border-radius:12px;box-shadow:0 0 0 1px var(--border);cursor:pointer;overflow:hidden;transition:box-shadow .15s;}
.p-card:hover{box-shadow:0 0 0 1px var(--control-border);}
...
/* ── HOUSEHOLDS ── */
.h-card{background:var(--white);border-radius:18px;box-shadow:0 0 0 1px var(--border);cursor:pointer;padding:16px 18px;transition:box-shadow .15s;}
.h-card:hover{box-shadow:0 0 0 1px var(--control-border);}
...
.badge-open{background:var(--success-bg);color:var(--success);padding:2px 8px;border-radius:99px;font-size:.68rem;font-weight:700;}
.badge-closed{background:var(--linen);color:var(--warm-gray);padding:2px 8px;border-radius:99px;font-size:.68rem;font-weight:700;}
.badge-warn{background:var(--error-bg);color:var(--error);padding:2px 8px;border-radius:99px;font-size:.68rem;font-weight:700;}
...
.field input,.field select,.field textarea{min-height:40px;padding:8px 12px;border:1px solid var(--control-border);border-radius:var(--radius-control);font-family:var(--font-body);font-size:15px;color:var(--text);background:var(--surface);}
...
.giv-txn-table-wrap{flex:1;min-height:0;overflow:auto;background:var(--white);border-radius:20px;box-shadow:0 0 0 1px var(--border);}
/* ── CHURCH REGISTER ── */
...
.reg-search{flex:1;min-width:160px;max-width:280px;padding:6px 10px;border:1px solid var(--border);border-radius:7px;font-size:13px;}
.reg-search:focus{border-color:var(--teal);}
.reg-year-select{padding:6px 10px;border:1px solid var(--border);border-radius:7px;font-size:13px;background:var(--white);cursor:pointer;}
...
.reg-cert-btn{background:none;border:none;color:var(--primary);cursor:pointer;font-size:.78rem;padding:2px 6px;border-radius:4px;opacity:.7;}
...
.report-tile{background:var(--white);border-radius:16px;box-shadow:0 0 0 1px var(--border);padding:20px;cursor:pointer;transition:box-shadow .15s;}
.report-tile:hover{box-shadow:0 0 0 1px var(--control-border);}
...
.report-output{background:var(--white);border-radius:20px;box-shadow:0 0 0 1px var(--border);padding:24px;display:none;}
...
.rpt-group-sub td{font-style:italic;font-weight:600;background:var(--page);border-bottom:1px solid var(--border) !important;}
...
.board-mode-toggle button.active{background:var(--tint);color:var(--primary);}
.board-kpi-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:16px;}
.board-kpi-card{background:var(--white);border-radius:18px;box-shadow:0 0 0 1px var(--border);padding:16px 18px;border-top:4px solid var(--color-teal);}
...
.board-card{background:var(--white);border-radius:18px;box-shadow:0 0 0 1px var(--border);padding:18px 20px;}
...
.board-empty{background:var(--white);border-radius:18px;box-shadow:0 0 0 1px var(--border);padding:40px;text-align:center;color:var(--warm-gray);font-size:.92rem;}
/* Narrative page (1B) */
...
.board-nv-body{font-size:14px;line-height:1.65;color:var(--text);}
...
.giv-queue-card{background:var(--white);border-radius:14px;box-shadow:0 0 0 1px var(--border);padding:13px 16px;border-left:4px solid var(--color-teal);}
...
.giv-badge-ok{background:var(--success-bg);color:var(--success);}
.giv-badge-warn{background:var(--page);color:var(--warning);}
.giv-badge-bad{background:var(--error-bg);color:var(--error);}
...
.giv-cover-track{height:9px;border-radius:5px;background:var(--page);overflow:hidden;display:flex;margin:10px 0 6px;}
...
.giv-dep-shortfall{background:var(--page);border:1px solid var(--border);border-radius:12px;padding:10px 14px;font-size:12.5px;color:var(--warning);display:flex;align-items:center;gap:12px;flex-wrap:wrap;}
.giv-dep-remove{background:none;border:none;color:var(--error);font-size:.76rem;font-weight:600;cursor:pointer;font-family:var(--font-body);padding:0;}
.giv-off-layout{display:grid;grid-template-columns:320px 1fr;gap:0;border-radius:20px;overflow:hidden;box-shadow:0 0 0 1px var(--border);min-height:520px;background:var(--white);}
...
.giv-off-row.selected{background:var(--info-bg);border-left-color:var(--color-teal);}
...
.att-root{
  --att-page:var(--page);--att-inset:var(--page);--att-hairline:var(--border);--att-table-hairline:var(--border);--att-track:var(--border);
  --att-text-2:var(--muted);--att-text-3:var(--muted);--att-text-lbl:var(--text);
  --att-gold-text:var(--warning);
  ...
  --att-pos:var(--success);--att-neg:var(--error);--att-pill-text:var(--warning);--att-pill-bg:var(--warning-bg);
...
.att-card{background:var(--white);border-radius:18px;box-shadow:0 0 0 1px var(--border);padding:20px;}
...
.att-input:focus{border-color:var(--primary);background:var(--white);}
...
.att-btn-primary{padding:12px 22px;background:var(--primary);color:var(--white);border-radius:10px;font-size:.94rem;font-weight:700;border:none;cursor:pointer;font-family:var(--font-body);}
...
/* ── MODAL ── */
.modal-overlay{position:fixed;inset:0;z-index:8000;background:var(--backdrop);display:none;align-items:center;justify-content:center;padding:16px;}
.modal-overlay.open{display:flex;}
.modal{background:var(--white);border:1px solid var(--border);border-radius:var(--radius-card);padding:24px;width:100%;max-width:560px;max-height:90vh;overflow-y:auto;box-shadow:none;}
@media(max-width:767px){.modal{padding:18px 16px;max-height:95vh;border-radius:10px;}}
.modal h2{font-family:var(--font-head);font-size:20px;font-weight:700;color:var(--brand-ink);margin-bottom:16px;}
...
.modal-section{font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.12em;color:var(--muted);margin:20px 0 8px;border-bottom:1px solid var(--border);padding-bottom:6px;}
...
.p-card.selected .p-select-cb{background:var(--primary);border-color:var(--primary);color:var(--white);}
/* ── SETTINGS ── */
...
:root{
  --sage-text:var(--success);--positive-on-navy:#8FD3A6;--negative-on-navy:#E8A088;
  --chip-positive-bg:var(--success-bg);--chip-warn-bg:var(--warning-bg);--chip-negative-bg:var(--error-bg);
}
...
.fin-kpi-card{background:var(--white);border-radius:18px;box-shadow:0 0 0 1px var(--border);padding:18px 20px;border-top:4px solid var(--color-teal);}
...
.fin-card{background:var(--white);border-radius:20px;box-shadow:0 0 0 1px var(--border);padding:22px 24px;}
...
.fin-balance-card{background:var(--white);border-radius:16px;box-shadow:0 0 0 1px var(--border);padding:16px 18px;display:flex;align-items:center;gap:12px;}
...
.fin-flow-toggle-btn.active{background:var(--white);color:var(--color-navy);box-shadow:0 0 0 1px var(--border);}
...
.fin-comp-radio{width:15px;height:15px;border-radius:50%;flex-shrink:0;background:var(--white);border:1.5px solid var(--border);}
...
.dash-stat{background:var(--white);border-radius:20px;box-shadow:0 0 0 1px var(--border);padding:22px 24px;display:flex;flex-direction:column;gap:4px;}
...
.dash-card{background:var(--white);border-radius:20px;box-shadow:0 0 0 1px var(--border);overflow:hidden;}
...
/* OS5 (2026-09-25) — Home. Every section collapses from its title and remembers that per
   browser; attendance entry and the month's birthdays/anniversaries/baptisms lead. */
.dsec{margin-bottom:16px;}
.dsec-hdr{display:flex;align-items:center;gap:8px;flex-wrap:wrap;min-height:44px;}
.dsec-h{margin:0;font-size:16px;font-weight:700;color:var(--text);}
.dsec-toggle{display:inline-flex;align-items:center;gap:8px;min-height:44px;background:none;border:none;padding:0 6px 0 0;font:inherit;font-size:inherit;font-weight:inherit;color:inherit;cursor:pointer;text-align:left;}
.dsec-toggle:hover{color:var(--primary);}
.dsec-chev{width:18px;height:18px;flex-shrink:0;stroke:currentColor;fill:none;stroke-width:2.2;stroke-linecap:round;stroke-linejoin:round;transform:rotate(90deg);transition:transform .15s;}
.dsec.is-collapsed .dsec-chev{transform:rotate(0deg);}
.dsec-meta{font-size:14px;font-weight:400;color:var(--muted);}
.dsec-actions{display:flex;align-items:center;gap:6px;margin-left:auto;flex-wrap:wrap;}
.dsec-body{margin-top:6px;}
.dsec-body[hidden]{display:none;}
.dsec-sub{margin:0;}
.dsec-sub .dsec-hdr{padding:4px 12px 4px 14px;border-bottom:1px solid var(--border);}
.dsec-sub .dsec-h{font-size:15px;}
.dsec-sub.is-collapsed .dsec-hdr{border-bottom:none;}
.dsec-sub .dsec-body{margin-top:0;}
.home-panel{background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:16px 18px;}
.home-att{display:grid;grid-template-columns:minmax(200px,1.6fr) repeat(2,minmax(90px,1fr)) auto auto;gap:14px;align-items:end;}
.home-att label{display:block;font-size:14px;font-weight:600;color:var(--text);margin-bottom:4px;}
.home-att select,.home-att input{width:100%;min-height:44px;}
.home-att-total{min-height:44px;display:flex;flex-direction:column;justify-content:flex-end;padding:0 4px;}
.home-att-total-lbl{font-size:14px;font-weight:600;color:var(--muted);}
.home-att-total-n{font-size:24px;font-weight:700;color:var(--text);line-height:1.2;font-variant-numeric:tabular-nums;}
.home-att-actions{display:flex;align-items:center;gap:10px;min-height:44px;}
.home-att-foot{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:10px;font-size:14px;color:var(--muted);}
.home-att-foot .os-link-btn{margin-left:auto;}
.home-month-nav{display:inline-flex;align-items:center;gap:2px;}
.home-month-arrow{width:44px;height:44px;padding:0;display:inline-flex;align-items:center;justify-content:center;}
.home-month-nav .home-month-lbl{min-width:120px;text-align:center;font-size:15px;font-weight:600;color:var(--text);}
.home-month-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;align-items:start;}
.home-month-card{background:var(--surface);border:1px solid var(--border);border-radius:12px;overflow:hidden;}
.home-month-list{list-style:none;margin:0;padding:4px 0;}
.home-month-list li{display:flex;align-items:baseline;gap:12px;padding:0 14px;}
.home-month-list li button{flex:1;min-width:0;min-height:40px;background:none;border:none;padding:0;font:inherit;font-size:15px;color:var(--text);text-align:left;cursor:pointer;}
.home-month-list li button:hover{color:var(--primary);text-decoration:underline;}
.home-month-date{flex-shrink:0;width:52px;font-size:14px;color:var(--muted);font-variant-numeric:tabular-nums;}
.home-month-empty{padding:12px 14px;font-size:14px;color:var(--muted);}
.home-copy{font-size:14px;}
.home-month-foot{display:flex;justify-content:flex-end;padding:0 10px 4px;border-top:1px solid var(--border);}
.home-more{display:flex;align-items:center;gap:12px;margin:28px 0 4px;padding-top:16px;border-top:1px solid var(--border);}
.home-more-h{margin:0;margin-right:auto;font-size:14px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;color:var(--muted);}
@media(max-width:1100px){.home-month-grid{grid-template-columns:repeat(2,minmax(0,1fr));}.home-att{grid-template-columns:1fr 1fr;}.home-att-field-date{grid-column:1 / -1;}}
@media(max-width:767px){.home-month-grid{grid-template-columns:1fr;}}
...
.pv-photo{width:88px;height:88px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:30px;font-weight:700;flex-shrink:0;box-shadow:0 0 0 1px var(--control-border);}
...
.pv-tab.active{color:var(--primary);border-bottom-color:var(--primary);font-weight:700;}
...
.pv-section{background:var(--warm-surface-card);border-radius:18px;box-shadow:0 0 0 1px var(--border);padding:20px 22px;margin-bottom:16px;}
...
.pv2-card{background:var(--warm-surface-card);border:1px solid var(--warm-divider);border-radius:16px;overflow:hidden;box-shadow:0 0 0 1px var(--border);scroll-margin-top:14px;}
...
.pv2-inp{width:100%;max-width:320px;font-size:15px;color:var(--color-navy);background:var(--white);border:1px solid var(--color-teal);border-radius:9px;padding:9px 11px;box-shadow:0 0 0 3px rgba(46,126,166,.15);font-family:var(--font-body);}
...
/* ── OPEN SKY PERSON PAGE (OS3, 2026-09-25) ──
   Overrides the pv/pv2 rules above for the person page. The pv2-card family is shared with the
   household and organization views, so those pick up the same flat, bordered look. */
#profile-view{background:var(--page);}
#profile-view .topbar{gap:16px;}
.topbar-back{display:inline-flex;align-items:center;gap:6px;min-height:44px;font-size:15px;font-weight:600;color:var(--link);text-decoration:none;}
.topbar-back:hover{color:var(--primary-hover);text-decoration:underline;}
.pv-topbar-name{font-size:15px;color:var(--muted);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;min-width:0;}
.pv-hdr{align-items:center;gap:24px;padding:24px 32px;border-bottom:1px solid var(--border);background:var(--page);flex-wrap:wrap;}
.pv-photo{background:var(--tint);color:var(--primary);box-shadow:none;border:1px solid var(--border);}
.pv-photo-edit-btn{background:var(--primary);width:32px;height:32px;}
.pv-photo-edit-btn:hover{background:var(--primary-hover);}
.pv-photo-menu{border:1px solid var(--border);border-radius:var(--radius-card);box-shadow:none;}
.pv-photo-menu button{font-size:15px;color:var(--text);min-height:40px;}
.pv-photo-menu button:hover{background:var(--page);}
.pv-photo-menu button.danger{color:var(--error);}
.pv-hdr-info{min-width:220px;}
.pv-fullname{font-family:var(--font-ui);font-size:32px;font-weight:700;color:var(--brand-ink);letter-spacing:-.025em;}
.pv-meta{font-size:16px;color:var(--muted);gap:8px;}
.pv-meta a{color:var(--link);font-weight:600;}
.pv-meta-sep{color:var(--muted);}
.pv-role-txt{font-size:16px;color:var(--muted);}
.pv-hdr-actions{flex-wrap:wrap;gap:8px;align-items:center;}
.pv-hdr-actions a.btn-secondary{text-decoration:none;}
.pv-more{position:relative;}
.pv-more-menu{position:absolute;right:0;top:calc(100% + 6px);z-index:60;min-width:240px;background:var(--surface);border:1px solid var(--control-border);border-radius:var(--radius-card);padding:6px;display:flex;flex-direction:column;}
.pv-more-menu[hidden]{display:none;}
.pv-more-item{display:block;width:100%;text-align:left;background:none;border:none;border-radius:6px;min-height:44px;padding:8px 12px;font:inherit;font-size:15px;color:var(--text);cursor:pointer;}
.pv-more-item:hover,.pv-more-item:focus-visible{background:var(--tint);color:var(--primary);}
.pv-more-sep{height:1px;background:var(--border);margin:6px 4px;}
.pv-tabs{padding:0 32px;background:var(--page);border-bottom:1px solid var(--border);}
.pv-tab{background:none;border:none;border-bottom:3px solid transparent;font:inherit;font-size:16px;font-weight:600;color:var(--muted);padding:12px 16px;min-height:44px;}
.pv-tab:hover{color:var(--primary);}
.pv-main{background:var(--surface);padding:32px;}
.pv2-layout{display:grid;grid-template-columns:minmax(0,2fr) minmax(280px,1fr);gap:32px;max-width:1280px;align-items:start;}
.pv2-main,.pv2-aside{display:flex;flex-direction:column;gap:24px;min-width:0;}
.pv2-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius-card);box-shadow:none;overflow:visible;scroll-margin-top:16px;}
.pv2-card-hd{padding:16px 24px;border-bottom:1px solid var(--border);gap:12px;}
.pv2-card-hd h2,.pv2-card-hd h3{margin:0;font-size:20px;font-weight:700;letter-spacing:0;text-transform:none;color:var(--brand-ink);}
.pv2-card-hd-tag{font-size:14px;font-weight:600;color:var(--muted);}
.pv2-card-btn{min-height:36px;padding:4px 12px;font-size:14px;}
.pv2-card-btn[hidden]{display:none;}
.pv2-card-bd{padding:4px 24px 8px;}
.pv2-card-bd.pad{padding:16px 24px 20px;}
.pv2-frow{padding:12px 0;border-bottom:1px solid var(--border);align-items:baseline;}
.pv2-flabel{width:150px;color:var(--muted);font-size:15px;font-weight:400;padding-top:0;}
.pv2-ro{font-size:16px;padding:0;color:var(--text);overflow-wrap:anywhere;}
.pv2-ro.empty{color:var(--muted);}
.pv2-sub{font-size:14px;color:var(--muted);}
.pv2-muted{color:var(--muted);}
.pv2-edit{padding:16px 0 8px;}
.pv2-edit-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px;}
.pv2-edit-field{display:flex;flex-direction:column;gap:8px;min-width:0;}
.pv2-edit-field label{font-size:14px;font-weight:600;color:var(--text);}
.pv2-inp{max-width:none;min-height:44px;font-size:16px;color:var(--text);background:var(--surface);border:1px solid var(--control-border);border-radius:var(--radius-control);padding:8px 12px;box-shadow:none;}
.pv2-inp:focus{border-color:var(--primary);}
.pv2-inp.sel{padding-right:32px;}
.pv2-date-row{display:flex;flex-direction:column;gap:8px;}
.pv2-prec{margin-top:0;font-size:14px;min-height:40px;color:var(--text);}
.pv2-check{display:flex;align-items:center;gap:10px;min-height:44px;font-size:16px;color:var(--text);grid-column:1/-1;cursor:pointer;}
.pv2-check input{width:20px;height:20px;accent-color:var(--primary);}
.pv2-edit-inline{display:flex;align-items:center;gap:12px;flex-wrap:wrap;margin-top:16px;}
.pv2-edit-note{font-size:14px;color:var(--muted);}
.pv2-edit-err{margin-top:16px;padding:12px 16px;border-radius:var(--radius-control);background:var(--error-bg);color:var(--error);border:1px solid var(--error);}
.pv2-edit-err[hidden]{display:none;}
.pv2-edit-actions{display:flex;align-items:center;justify-content:flex-end;gap:12px;flex-wrap:wrap;margin-top:20px;padding-top:16px;border-top:1px solid var(--border);}
.pv2-save-state{margin-right:auto;}
.pv2-extra-row{display:flex;align-items:center;gap:12px;flex-wrap:wrap;padding:12px 0;border-bottom:1px solid var(--border);}
.pv2-extra-row .pv2-flabel{padding-top:0;}
.pv2-extra-actions{display:flex;gap:8px;flex-wrap:wrap;padding:16px 0 8px;}
.pv2-toggle{min-height:36px;padding:4px 12px;border-radius:var(--radius-control);border:1px solid var(--control-border);background:var(--surface);color:var(--text);font:inherit;font-size:14px;font-weight:600;cursor:pointer;}
.pv2-toggle[aria-pressed="true"]{background:var(--success-bg);border-color:var(--success);color:var(--success);}
.pv2-toggle:hover:not(:disabled){background:var(--tint);color:var(--primary);}
.pv2-toggle:disabled{opacity:.6;cursor:default;}
.pv2-chip{background:var(--tint);color:var(--primary);font-size:14px;border-radius:var(--radius-control);padding:4px 4px 4px 12px;min-height:32px;}
.pv2-chip-x{background:none;color:var(--primary);width:28px;height:28px;border-radius:6px;}
.pv2-chip-x:hover{background:var(--surface);}
.pv2-chip-x svg{width:16px;height:16px;fill:none;stroke:currentColor;stroke-width:2;stroke-linecap:round;}
.pv2-chip-add{border:1px solid var(--control-border);background:var(--surface);color:var(--primary);font-size:14px;border-radius:var(--radius-control);min-height:36px;padding:4px 12px;}
.pv2-chip-add:hover{background:var(--tint);}
.pv2-dir-line{margin:0 0 8px;}
.pv2-dir-note{color:var(--muted);margin:0;}
.pv2-fu-item{display:flex;align-items:flex-start;gap:12px;padding:12px 16px;margin-bottom:8px;border-radius:var(--radius-card);background:var(--warning-bg);border:1px solid var(--border);}
.pv2-hh-name{display:inline-block;font-size:17px;font-weight:700;color:var(--link);margin-bottom:8px;}
.pv-family-member{gap:12px;padding:10px 0;border-bottom:1px solid var(--border);}
.pv-family-avatar{width:36px;height:36px;font-size:13px;background:var(--tint);color:var(--primary);}
.pv-family-name{font-size:15px;font-weight:600;color:var(--brand-ink);}
.pv-family-name[aria-current]{font-weight:700;}
.pv-family-link{background:none;border:none;padding:0;font:inherit;font-size:15px;font-weight:600;color:var(--link);cursor:pointer;text-align:left;}
.pv-family-link:hover{text-decoration:underline;color:var(--primary-hover);}
.pv-family-meta{font-size:14px;color:var(--muted);}
.pv2-tile{background:var(--page);border:1px solid var(--border);border-radius:var(--radius-control);}
.pv2-tile-lbl{font-size:13px;font-weight:600;text-transform:none;letter-spacing:0;color:var(--muted);}
.pv2-tile-val{font-size:22px;font-weight:700;color:var(--brand-ink)!important;font-variant-numeric:tabular-nums;}
.pv2-gift{border-bottom:1px solid var(--border);}
.pv2-toast{background:var(--brand-ink);border-radius:var(--radius-control);box-shadow:none;}
[id^="pv-map-"]>div,[id^="hv-map-"]>div{line-height:1.5;}
@media(max-width:1100px){
  .pv2-layout{grid-template-columns:1fr;}
  .pv-hdr,.pv-main{padding-left:24px;padding-right:24px;}
  .pv-tabs{padding:0 24px;}
}
@media(max-width:767px){
  .pv-hdr{padding:16px;gap:16px;}
  .pv-main{padding:16px;}
  .pv-tabs{padding:0 8px;}
  .pv-fullname{font-size:28px;}
  .pv-more-menu{left:0;right:auto;}
  .pv2-edit-grid{grid-template-columns:1fr;}
  .pv2-flabel{width:110px;}
  .pv2-card-hd,.pv2-card-bd,.pv2-card-bd.pad{padding-left:16px;padding-right:16px;}
}
/* Add person — the short form as a side panel (Timothy Workspace drawer editing). */
#person-modal.os-drawer-overlay{justify-content:flex-end;align-items:stretch;padding:0;}
#person-modal .os-drawer{max-width:480px;height:100%;max-height:none;border-radius:0;border-left:1px solid var(--border);padding:24px;}
#person-modal:not(.pm-full) .pm-extra{display:none!important;}
.os-drawer-intro{color:var(--muted);margin:-8px 0 16px;}
@media(max-width:767px){#person-modal .os-drawer{max-width:100%;border-left:none;}}
...
@media(max-width:767px){
  ...
  .vol-subtab-btn{white-space:nowrap;flex-shrink:0;padding:8px;font-size:14px;}
...
.ev-list-header h4{font-family:var(--font-head);font-weight:600;font-size:1rem;color:var(--color-navy);margin:0 0 10px;}
...
.ev-list-footer button{width:100%;background:var(--primary);color:var(--white);border:none;border-radius:8px;padding:9px;font-size:.82rem;font-weight:600;cursor:pointer;}
.ev-new-btn{background:var(--primary);color:var(--white);border:none;border-radius:7px;padding:6px 11px;font-size:.75rem;font-weight:600;cursor:pointer;flex-shrink:0;}
...
.ev-btn-primary{background:var(--primary);color:var(--white);border:none;border-radius:8px;padding:10px 20px;font-size:.82rem;font-weight:600;cursor:pointer;}
...
.ev-day-header h4{font-family:var(--font-head);font-weight:600;font-size:.92rem;color:var(--color-navy);margin:0;}
...
.status-pill.status-new{background:rgba(201,151,58,.15);color:var(--warning);}
...
<nav class="sidebar" id="sidebar" aria-label="Main">
  ...
  <div role="link" tabindex="0" class="s-item active no-member" data-tab="home" onclick="showTab('home')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 21v-8a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v8"/><path d="M3 10a2 2 0 0 1 .709-1.528l7-5.999a2 2 0 0 1 2.582 0l7 5.999A2 2 0 0 1 21 10v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg><span class="s-tip">Home</span></div>
  <div class="s-section-hdr">People</div>
  <div role="link" tabindex="0" class="s-item" data-tab="people" onclick="showTab('people')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg><span class="s-tip">People</span></div>
  <div role="link" tabindex="0" class="s-item no-member" data-tab="households" onclick="showTab('households')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M22 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg><span class="s-tip">Households</span></div>
  <div role="link" tabindex="0" class="s-item no-member" data-tab="organizations" onclick="showTab('organizations')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 22V4a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v18Z"/><path d="M6 12H4a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h2"/><path d="M18 9h2a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2h-2"/><path d="M10 6h4"/><path d="M10 10h4"/><path d="M10 14h4"/><path d="M10 18h4"/></svg><span class="s-tip">Organizations</span></div>
  <div class="s-section-hdr require-finance" id="s-hdr-finance">Finance</div>
  <div role="link" tabindex="0" class="s-item require-finance" data-tab="giving" onclick="showTab('giving')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M11 15h2a2 2 0 1 0 0-4h-3c-.6 0-1.1.2-1.4.6L3 17"/><path d="m7 21 1.6-1.4c.3-.4.8-.6 1.4-.6h4c1.1 0 2.1-.4 2.8-1.2l4.6-4.4a2 2 0 0 0-2.75-2.91l-4.2 3.9"/><path d="m2 16 6 6"/><circle cx="16" cy="9" r="2.9"/><circle cx="6" cy="5" r="3"/></svg><span class="s-tip">Giving</span></div>
  <div role="link" tabindex="0" class="s-item require-tuitionaid" data-tab="tuitionaid" onclick="showTab('tuitionaid')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M21.42 10.922a1 1 0 0 0-.019-1.838L12.83 5.18a2 2 0 0 0-1.66 0L2.6 9.08a1 1 0 0 0 0 1.832l8.57 3.908a2 2 0 0 0 1.66 0z"/><path d="M22 10v6"/><path d="M6 12.5V16a6 3 0 0 0 12 0v-3.5"/></svg><span class="s-tip">Tuition Aid</span></div>
  <div role="link" tabindex="0" class="s-item require-financeov" data-tab="finance" onclick="openFinanceWorkspace()"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M21 12c.552 0 1.005-.449.95-.998a10 10 0 0 0-8.953-8.951c-.55-.055-.998.398-.998.95v8a1 1 0 0 0 1 1z"/><path d="M21.21 15.89A10 10 0 1 1 8 2.83"/></svg><span class="s-tip">Financial Reports</span></div>
  <div class="s-section-hdr no-member">Ministry</div>
  <div role="link" tabindex="0" class="s-item require-attendance" data-tab="attendance" onclick="showTab('attendance')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 2v4"/><path d="M16 2v4"/><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18"/><path d="m9 16 2 2 4-4"/></svg><span class="s-tip">Attendance</span></div>
  <div role="link" tabindex="0" class="s-item no-member require-reports" data-tab="reports" onclick="showTab('reports')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 3v16a2 2 0 0 0 2 2h16"/><path d="M18 17V9"/><path d="M13 17V5"/><path d="M8 17v-3"/></svg><span class="s-tip">Reports</span></div>
  <div role="link" tabindex="0" class="s-item require-register" data-tab="register" onclick="showTab('register')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 7v14"/><path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z"/></svg><span class="s-tip">Register</span></div>
  <div class="s-section-hdr require-admin">Admin</div>
  <div role="link" tabindex="0" class="s-item require-admin" data-tab="volunteers" onclick="showTab('volunteers')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M11 14h2a2 2 0 1 0 0-4h-3c-.6 0-1.1.2-1.4.6L3 16"/><path d="m7 20 1.6-1.4c.3-.4.8-.6 1.4-.6h4c1.1 0 2.1-.4 2.8-1.2l4.6-4.4a2 2 0 0 0-2.75-2.91l-4.2 3.9"/><path d="m2 15 6 6"/><path d="M19.5 8.5c.7-.7 1.5-1.6 1.5-2.7A2.73 2.73 0 0 0 16 4a2.78 2.78 0 0 0-5 1.8c0 1.2.8 2 1.5 2.8L16 12Z"/></svg><span class="s-tip">Volunteers</span></div>
  <div role="link" tabindex="0" class="s-item require-admin" data-tab="scheduler" onclick="showTab('scheduler')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 2v4"/><path d="M16 2v4"/><rect width="18" height="18" x="3" y="4" rx="2"/><path d="M3 10h18"/><path d="M8 14h.01"/><path d="M12 14h.01"/><path d="M16 14h.01"/><path d="M8 18h.01"/><path d="M12 18h.01"/><path d="M16 18h.01"/></svg><span class="s-tip">Scheduler</span></div>
  <div class="s-bottom">
    <div role="link" tabindex="0" class="s-item require-admin" data-tab="settings" onclick="showTab('settings')"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12.22 2h-.44a2 2 0 0 0-2 2v.18a2 2 0 0 1-1 1.73l-.43.25a2 2 0 0 1-2 0l-.15-.08a2 2 0 0 0-2.73.73l-.22.38a2 2 0 0 0 .73 2.73l.15.1a2 2 0 0 1 1 1.72v.51a2 2 0 0 1-1 1.74l-.15.09a2 2 0 0 0-.73 2.73l.22.38a2 2 0 0 0 2.73.73l.15-.08a2 2 0 0 1 2 0l.43.25a2 2 0 0 1 1 1.73V20a2 2 0 0 0 2 2h.44a2 2 0 0 0 2-2v-.18a2 2 0 0 1 1-1.73l.43-.25a2 2 0 0 1 2 0l.15.08a2 2 0 0 0 2.73-.73l.22-.39a2 2 0 0 0-.73-2.73l-.15-.08a2 2 0 0 1-1-1.74v-.5a2 2 0 0 1 1-1.74l.15-.09a2 2 0 0 0 .73-2.73l-.22-.38a2 2 0 0 0-2.73-.73l-.15.08a2 2 0 0 1-2 0l-.43-.25a2 2 0 0 1-1-1.73V4a2 2 0 0 0-2-2z"/><circle cx="12" cy="12" r="3"/></svg><span class="s-tip">Settings</span></div>
    <a class="s-item" href="?mobile=1" title="Switch to the phone-optimized view"><svg viewBox="0 0 24 24" aria-hidden="true"><rect width="14" height="20" x="5" y="2" rx="2" ry="2"/><path d="M12 18h.01"/></svg><span class="s-tip">Mobile View</span></a>
...
<div class="topbar">
  ...
  <div class="topbar-brand" onclick="showTab('home')" title="Home"><img class="topbar-mark" src="/icons/connect-mark.png" alt="" width="32" height="32"><span class="topbar-word">Connect</span></div>
  <h1 class="topbar-title" id="topbar-title">People</h1>
  <div style="display:flex;gap:12px;align-items:center;">
    <span class="topbar-ver" id="deploy-ver"></span>
