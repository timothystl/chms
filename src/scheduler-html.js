...
/* ── Print Preview (SC7): Single Sunday / Full Month / Bulletin Insert ──
   Clicking Print no longer prints the on-screen grid directly — it opens this
   overlay so staff can pick a purpose-built layout first. The old grid-based
   @media print rules above stay in place as a harmless fallback for a raw
   Ctrl+P outside this flow. Per-mode @page sizing (portrait letter for Single
   Sunday, landscape letter for Full Month, a fixed 7in×8.5in half-sheet for
   Bulletin Insert) is applied by rewriting #pp-page-size-style's content in
   JS rather than with CSS custom properties — @page does not reliably accept
   var() for the size property across browsers. */
#print-preview-overlay { display: none; }
#print-preview-overlay.pp-open {
  display: block; position: fixed; inset: 0; z-index: 400; overflow-y: auto;
  background: #EDE7DA; font-family: 'DM Sans','Source Sans 3',Arial,sans-serif;
}
.pp-toolbar-wrap { max-width: 900px; margin: 0 auto; padding: 22px 24px 0; }
.pp-toolbar { display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 14px;
  background: #1E2D4A; border-radius: 12px; padding: 14px 20px; box-shadow: 0 2px 12px rgba(10,60,92,0.15); }
...
.pp-seg-btn { border: none; border-radius: 999px; padding: 7px 16px; font-size: .82rem; font-weight: 700;
  cursor: pointer; font-family: inherit; background: transparent; color: #7A6E60; }
...
.pp-close-btn { background: none; border: none; color: rgba(255,255,255,.75); font-size: 1.3rem; cursor: pointer; padding: 0 6px; line-height: 1; }
.pp-close-btn:hover { color: #fff; }
.pp-hint { text-align: center; color: #7A6E60; font-size: .8rem; margin: 10px 0 0; }
.pp-page-wrap { display: flex; justify-content: center; padding: 24px 24px 60px; }
...
@media (max-width: 900px) {
  .pp-page-single, .pp-page-month, .pp-page-bulletin { width: 100%; min-width: 0; }
}
@media print {
...
<!-- ══ PRINT PREVIEW (SC7) ═══════════════════════════════════════════════════
     Content is rendered entirely by JS (renderPrintPreview / ppBuildPageHtml) —
     see openPrintPreview() near the schedule-tab print button handler. -->
<div id="print-preview-overlay"></div>
...
function ppUpdatePageSize() {
  var el = document.getElementById('pp-page-size-style');
  if (!el) return;
...
function ppBuildSingleHtml(d) {
  var html = '<div class="pp-page pp-page-single">'
    ...
    +   '<div style="font-size:11.5px;font-weight:700;letter-spacing:.14em;text-transform:uppercase;color:#C9973A;margin-bottom:10px;">Timothy Lutheran Church &middot; Serving This Week</div>'
...
function ppBuildMonthHtml() {
  ...
  var html = '<div class="pp-page pp-page-month">'
    ...
    +   '<div style="font-size:20.8px;font-weight:800;color:#1E2D4A;">Serving Schedule &mdash; ' + esc(monthKeyLabel(currentMonthKey)) + '</div>'
    + '</div>'
...
function renderPrintPreview() {
  ...
  var sundayOptions = ppSundayRows.map(function(row, i) {
    var d = ppSundayData(row);
    return '<option value="' + i + '"' + (i === ppSelectedIdx ? ' selected' : '') + '>' + esc(d.dateLong) + ' &mdash; ' + esc(d.ordinalLabel) + '</option>';
  }).join('');

  overlay.innerHTML =
    '<div class="pp-toolbar-wrap">'
      + '<div class="pp-toolbar">'
        ...
        + '<div style="display:flex;align-items:center;gap:10px;">'
          ...
          +   '<button type="button" class="pp-close-btn" id="pp-close-btn" title="Close print preview">&times;</button>'
        + '</div>'
      + '</div>'
      + '<p class="pp-hint">This toolbar is hidden when printing &mdash; only the sheet below prints.</p>'
    + '</div>'
    + '<div class="pp-page-wrap">' + ppBuildPageHtml() + '</div>';

  var sel = document.getElementById('pp-sunday-select');
  if (sel) sel.addEventListener('change', function(e) {
    ppSelectedIdx = parseInt(e.target.value, 10) || 0;
    renderPrintPreview();
  });
  overlay.querySelectorAll('[data-pp-mode]').forEach(function(btn) {
  ...
  if (printBtn) printBtn.addEventListener('click', ppDoPrint);
  var closeBtn = document.getElementById('pp-close-btn');
  if (closeBtn) closeBtn.addEventListener('click', closePrintPreview);
}

function ppDoPrint() { window.print(); }

// Imperatively hides every other sibling in the app-content wrapper for the
// duration of printing, then restores them. Done in JS via real beforeprint/
// afterprint events (not a body.pp-printing CSS class, and not just wrapping
// the window.print() call above) for two reasons: (1) in embedded mode
// (scheduler-inline.js) this script's document.body is the whole ChMS page,
// not the .sched-root wrapper, so a body-scoped selector would silently
// never match there; (2) window.print() is not guaranteed synchronous across
// browsers (Safari in particular can return before the dialog closes), so
// restoring right after the call risks un-hiding content before it prints.
var _ppHiddenEls = [];
window.addEventListener('beforeprint', function() {
  var overlay = document.getElementById('print-preview-overlay');
  if (!overlay || !overlay.classList.contains('pp-open')) return;
  var appContent = document.getElementById('sched-app-content') || document.getElementById('app-content');
  if (!appContent) return;
  _ppHiddenEls = [];
  Array.prototype.forEach.call(appContent.children, function(child) {
    if (child !== overlay && child.style.display !== 'none') {
      _ppHiddenEls.push(child);
      child.setAttribute('data-pp-hidden', '1');
      child.style.setProperty('display', 'none', 'important');
    }
  });
});
window.addEventListener('afterprint', function() {
  _ppHiddenEls.forEach(function(child) {
    child.style.removeProperty('display');
    child.removeAttribute('data-pp-hidden');
  });
  _ppHiddenEls = [];
});
...
function openPrintPreview() {
  ...
  ppMode = 'single';
  renderPrintPreview();
  document.getElementById('print-preview-overlay').classList.add('pp-open');
}
