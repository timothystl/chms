...
export const LOGIN_HTML = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Sign In \u2014 Connect</title><link rel="icon" type="image/png" sizes="32x32" href="/icons/icon-32.png"><link rel="apple-touch-icon" sizes="180x180" href="/icons/icon-180.png"><link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@1,300&family=DM+Sans:wght@400;500;600&display=swap" rel="stylesheet"><style>:root{--navy:#1E2D4A;--teal:#2E7EA6;--gold:#C9973A;--cream:#F8F4EE;--muted:#8A8898;--val-welcome:#4DAA6B;--val-receive:#1D63B3;--val-grow:#21A9B3;--val-go:#F5B731;}*{box-sizing:border-box;margin:0;padding:0;}body{font-family:'DM Sans',sans-serif;font-weight:400;background:var(--cream);display:flex;align-items:center;justify-content:center;min-height:100vh;}.card{background:#fff;border-radius:16px;padding:2.5rem;max-width:380px;width:100%;box-shadow:0 4px 24px rgba(30,45,74,.12);}.wm{display:flex;flex-direction:column;align-items:center;text-align:center;margin-bottom:1.75rem;}.wm-mark{width:78px;height:78px;display:block;margin-bottom:1rem;}.wm-display{font-family:'DM Sans',sans-serif;font-weight:600;font-size:2.3rem;letter-spacing:.07em;color:var(--navy);line-height:1;}.wm-rule{width:200px;height:2px;background:var(--navy);margin:.5rem 0 .45rem;}.wm-org{font-size:11px;font-weight:600;letter-spacing:.19em;text-transform:uppercase;color:var(--teal);}.wm-tag{font-family:'Cormorant Garamond',serif;font-style:italic;font-weight:300;font-size:1.02rem;color:var(--muted);margin-top:.55rem;}.field{margin-bottom:1rem;}label{display:block;font-size:10px;font-weight:500;text-transform:uppercase;letter-spacing:.2em;color:var(--navy);margin-bottom:.4rem;}input{width:100%;padding:.7rem 1rem;border:1.5px solid rgba(30,45,74,.2);border-radius:8px;font-size:.95rem;font-family:inherit;outline:none;}input:focus{border-color:var(--teal);}.btn{width:100%;background:var(--navy);color:#fff;border:none;padding:.85rem;border-radius:8px;font-size:1rem;font-weight:500;cursor:pointer;margin-top:.5rem;transition:background .15s;font-family:inherit;}.btn:hover{background:var(--teal);}.btn:disabled{opacity:.6;cursor:wait;}.hint{font-size:.78rem;color:#aaa;margin-top:1.2rem;text-align:center;border-top:1px solid #eee;padding-top:.9rem;}</style></head><body><div class="card"><div class="wm"><svg class="wm-mark" viewBox="0 0 512 512" role="img" aria-label="Connect"><g fill="none" stroke-width="54" stroke-linecap="round" stroke-linejoin="round"><path d="M92 200 L92 92 L200 92 M92 256 L170 256" stroke="var(--val-welcome)"/><path d="M312 92 L420 92 L420 200 M256 92 L256 170" stroke="var(--val-receive)"/><path d="M420 312 L420 420 L312 420 M420 256 L342 256" stroke="var(--val-grow)"/><path d="M200 420 L92 420 L92 312 M256 420 L256 342" stroke="var(--val-go)"/></g><circle cx="256" cy="256" r="95" fill="#fff"/><g fill="var(--navy)"><rect x="222" y="184" width="13" height="46" rx="2"/><rect x="208" y="196" width="41" height="13" rx="2"/><rect x="205" y="226" width="47" height="102" rx="3"/><path d="M252 246 L306 300 L294 300 L294 328 L252 328 Z"/></g><g fill="#fff"><rect x="219" y="248" width="18" height="22" rx="4"/><rect x="219" y="284" width="18" height="22" rx="4"/></g></svg><div class="wm-display">CONNECT</div><div class="wm-rule"></div><div class="wm-org">Timothy Lutheran Church</div><div class="wm-tag">From our Neighborhood to the Nations</div></div><!--ERROR--><form method="POST" action="/admin/login" onsubmit="var b=this.querySelector('.btn');b.disabled=true;b.textContent='Signing in\u2026';"><div class="field"><label for="un">Username</label><input type="text" id="un" name="username" placeholder="Enter username" autocomplete="username" autofocus required></div><div class="field"><label for="pw">Password</label><input type="password" id="pw" name="password" placeholder="Enter password" autocomplete="current-password" required></div><button class="btn" type="submit">Sign In</button></form><div style="margin-top:.9rem;text-align:center;"><a href="#" id="fp-link" style="color:#2E7EA6;font-size:.82rem;text-decoration:none;">Forgot password?</a></div><div id="fp-panel" style="display:none;margin-top:.9rem;padding-top:.9rem;border-top:1px solid #eee;"><p style="font-size:.82rem;color:#3D3530;margin-bottom:.6rem;">Enter your username or email. If an account exists, we'll send a reset link.</p><form id="fp-form" onsubmit="event.preventDefault();var f=this,b=f.querySelector('.btn');b.disabled=true;b.textContent='Sending…';fetch('/admin/forgot-password',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded'},body:'username='+encodeURIComponent(f.username.value)}).then(function(){document.getElementById('fp-panel').innerHTML='<div style=\\'padding:.7rem;background:#e8f6ed;color:#1d6b3a;border-radius:8px;font-size:.85rem;text-align:center;\\'>If an account exists, a reset email has been sent.</div>';});"><div class="field"><label for="fp-un">Username or email</label><input type="text" id="fp-un" name="username" required></div><button class="btn" type="submit">Send reset link</button></form></div><script>document.getElementById('fp-link').addEventListener('click',function(e){e.preventDefault();var p=document.getElementById('fp-panel');p.style.display=p.style.display==='none'?'block':'none';});</script></div></body></html>`;
