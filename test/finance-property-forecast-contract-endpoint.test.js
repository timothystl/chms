...
// ── Access gate: same sentinel-DB technique as test/finance-property-operating-contract-endpoint.test.js ──
...
const SEG = 'contracts/finance-property-forecast-v1';
...
describe('contracts/finance-property-forecast-v1 access gate', () => {
...
CREATE TABLE IF NOT EXISTS finance_property_budget_monthly (
  property_key TEXT NOT NULL DEFAULT 'ivanhoe', period TEXT NOT NULL,
  revenue_cents INTEGER NOT NULL DEFAULT 0, expenses_cents INTEGER NOT NULL DEFAULT 0,
  net_income_cents INTEGER NOT NULL DEFAULT 0, source TEXT NOT NULL DEFAULT 'ahra_import',
  updated_at TEXT NOT NULL DEFAULT (datetime('now')), PRIMARY KEY (property_key, period)
);
`;
...
describe('contracts/finance-property-forecast-v1 real handler', () => {
  it('answers with a valid, empty contract (not a 500) when the requested property has no rows yet', async () => {
    ...
    expect(body.forecastYear).toBeNull();
  });

  it('reflects real rows for the default (ivanhoe) property and selects the complete current-year plan', async () => {
    const db = makeTestDb();
    for (let i = 1; i <= 12; i++) {
      db._raw.prepare(`INSERT INTO finance_property_budget_monthly (property_key,period,revenue_cents,expenses_cents,net_income_cents,source) VALUES (?,?,?,?,?,?)`)
        .run('ivanhoe', `2026-${String(i).padStart(2, '0')}`, 979775, 462704, 517071, 'ahra_import');
    }
    const env = { DB: db };
    const url = new URL(`https://connect.example/admin/api/${SEG}`);
    const req = { json: async () => ({}), headers: { get: () => null } };
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.contract).toBe('connect.finance-property-forecast.v1');
    expect(body.propertyKey).toBe('ivanhoe');
    expect(body.periods).toHaveLength(12);
    expect(body.forecastYear).toBe(2026);
  });

  it('honors an explicit property_key query parameter', async () => {
    const db = makeTestDb();
    db._raw.prepare(`INSERT INTO finance_property_budget_monthly (property_key,period,revenue_cents,expenses_cents,net_income_cents,source) VALUES (?,?,?,?,?,?)`)
      .run('other', '2026-06', 100, 40, 60, 'ahra_import');
    const env = { DB: db };
