...
const PATH = '/api/contracts/finance-church-balances-multi-year-xlsx-import-v1';
...
// A "Statement of Financial Position" multi-year export: one column per fiscal year (bare
// "2026"/"2027" headers). Unlike the Income Statement multi-year parsers, a classification header
// row (Assets/Liabilities/Equity) itself carries a real balance figure, not a $0 placeholder --
// three classifications x two leaf rows each (the header row itself + one child) x two years =
// 12 total rows.
function buildFinancialPositionMultiYearXlsx({ sheetName = 'Sheet1' } = {}) {
...
${xmlRow(3, [cellStr('A3', 'Assets'), cellNum('B3', 5000), cellNum('C3', 5500)])}
${xmlRow(4, [cellStr('A4', '   Cash'), cellNum('B4', 5000), cellNum('C4', 5500)])}
${xmlRow(5, [cellStr('A5', 'Liabilities'), cellNum('B5', 1000), cellNum('C5', 1100)])}
${xmlRow(6, [cellStr('A6', '   Accounts Payable'), cellNum('B6', 1000), cellNum('C6', 1100)])}
${xmlRow(7, [cellStr('A7', 'Equity'), cellNum('B7', 4000), cellNum('C7', 4400)])}
${xmlRow(8, [cellStr('A8', '   Net Assets'), cellNum('B8', 4000), cellNum('C8', 4400)])}
</sheetData></worksheet>`;
...
describe('POST /api/contracts/finance-church-balances-multi-year-xlsx-import-v1', () => {
  ...
  it('previews without writing, then commits only the selected balance rows', async () => {
    ...
    const commitRes = await post({ env: baseEnv(db), token, path: '/api/contracts/finance-church-balances-multi-year-xlsx-commit-v1', body: { years: preview.years, rows: preview.rows.slice(0, 1) } });
  ...
  it('saves real rows for a finance-role user, tagged source=import for both fiscal years', async () => {
    ...
    expect(out).toMatchObject({ ok: true, years: [2026, 2027], imported: 12, savedBy: 'sarah' });

    const rows = db._raw.prepare(`SELECT * FROM finance_church_balances WHERE source='import'`).all();
    expect(rows).toHaveLength(12);
    const cash2027 = rows.find((r) => r.account_name === 'Cash' && r.fiscal_year === 2027);
    expect(cash2027.own_balance_cents).toBe(550000);
    expect(cash2027.category_path).toBe('Assets:Cash');
    const assets2026 = rows.find((r) => r.category_path === 'Assets' && r.fiscal_year === 2026);
    expect(assets2026.own_balance_cents).toBe(500000);

    const logRow = db._raw.prepare(`SELECT note FROM finance_import_log WHERE importer_key='church_balance_multi'`).get();
