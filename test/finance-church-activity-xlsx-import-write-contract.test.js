...
const PATH = '/api/contracts/finance-church-activity-xlsx-import-v1';
...
// A "Statement of Activity" multi-year export: one column per fiscal year (bare "2026"/"2027"
// headers), Actual only. 'Income'/'Expenses' header rows each have children, so they also
// produce a $0 flat row per year -- four accounts x two years = 8 total rows.
function buildActivityMultiYearXlsx({ sheetName = 'Sheet1' } = {}) {
...
${xmlRow(2, [cellStr('B2', '2026'), cellStr('C2', '2027')])}
${xmlRow(3, [cellStr('A3', 'Income')])}
${xmlRow(4, [cellStr('A4', '   Offerings'), cellNum('B4', 1000), cellNum('C4', 1100)])}
${xmlRow(5, [cellStr('A5', 'Expenses')])}
${xmlRow(6, [cellStr('A6', '   Utilities'), cellNum('B6', 200), cellNum('C6', 220)])}
</sheetData></worksheet>`;
...
describe('POST /api/contracts/finance-church-activity-xlsx-import-v1', () => {
  ...
  it('previews without writing, then commits only the selected activity rows', async () => {
    const db = makeTestDb();
    ...
    const previewRes = await post({ env: baseEnv(db), token, path: '/api/contracts/finance-church-activity-xlsx-preview-v1', body: { file_base64: bytesToBase64(buildActivityMultiYearXlsx()) } });
    expect(previewRes.status).toBe(200);
    const preview = await previewRes.json();
    expect(preview).toMatchObject({ ok: true, years: [2026, 2027] });
    expect(db._raw.prepare('SELECT * FROM finance_church_entries').all()).toHaveLength(0);
    const commitRes = await post({ env: baseEnv(db), token, path: '/api/contracts/finance-church-activity-xlsx-commit-v1', body: { years: preview.years, rows: preview.rows.slice(0, 1) } });
  ...
  it('saves real rows for a finance-role user, tagged source=import_activity, actual only', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('sarah@timothystl.org'));
    const fileBase64 = bytesToBase64(buildActivityMultiYearXlsx());

    const res = await post({ env: baseEnv(db), token, body: { file_base64: fileBase64 } });
    expect(res.status).toBe(200);
    const out = await res.json();
    expect(out).toMatchObject({ ok: true, years: [2026, 2027], imported: 8, savedBy: 'sarah' });

    const rows = db._raw.prepare(`SELECT * FROM finance_church_entries WHERE source='import_activity'`).all();
    expect(rows).toHaveLength(8);
    const util2027 = rows.find((r) => r.account_name === 'Utilities' && r.fiscal_year === 2027);
    expect(util2027.own_actual_cents).toBe(22000);
    expect(util2027.own_budget_cents).toBeNull();

    const logRow = db._raw.prepare(`SELECT note FROM finance_import_log WHERE importer_key='church_activity_multi'`).get();
    expect(logRow.note).toBe('2026, 2027');
  });
  ...
  it('allows admin unconditionally', async () => {
    ...
    const res = await post({ env: baseEnv(db), token, body: { file_base64: bytesToBase64(buildActivityMultiYearXlsx()) } });
    expect(res.status).toBe(200);
  });
  ...
  it('rejects a council-role user -- council defaults to none on the finance item too', async () => {
    const db = makeTestDb();
  ...
  it('rejects when the shared X-Contract-Key is wrong, before ever looking at identity', async () => {
    ...
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('sarah@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, contractKey: 'wrong-secret', body: { file_base64: bytesToBase64(buildActivityMultiYearXlsx()) } });
  ...
  it('rejects a missing or invalid Access assertion even with a correct contract key', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const noToken = await post({ env: baseEnv(db), body: { file_base64: bytesToBase64(buildActivityMultiYearXlsx()) } });
    expect(noToken.status).toBe(401);
    const garbage = await post({ env: baseEnv(db), token: 'not-a-jwt', body: { file_base64: bytesToBase64(buildActivityMultiYearXlsx()) } });
  ...
  it('returns 503 when Connect has not been configured with the Access team/audience yet', async () => {
    ...
    const res = await post({ env, token, body: { file_base64: bytesToBase64(buildActivityMultiYearXlsx()) } });
