...
// finance-property-operating-v1 (so Operating results renders live and the per-row Remove button
// has a real row to sit next to) -- the same two calls a real page load makes.
...
function postForm(env, { accessJwt, body } = {}) {
  ...
  return worker.fetch(new Request('https://finance.test/api/v1/connect-property-monthly-remove', {
    method: 'POST',
    headers: {
...
describe('Commercial Property — per-row monthly-entry Remove action and relay route', () => {
  it('rejects the wrong method on the write-only route with an Allow header naming only POST', async () => {
    const res = await worker.fetch(new Request('https://finance.test/api/v1/connect-property-monthly-remove'), baseEnv);
  ...
  it('does not show the Remove action when the viewer role is not verified', async () => {
    ...
    expect(html).not.toContain('/api/v1/connect-property-monthly-remove');
  });
  ...
  it('shows a per-row Remove action for a verified admin viewer', async () => {
    const env = roleEnv('admin', async () => new Response('not found', { status: 404 }));
    const res = await worker.fetch(new Request('https://finance.test/?section=property&page=operating-results', {
      headers: { 'Cf-Access-Jwt-Assertion': 'signed.jwt.here' },
    }), env);
    const html = await res.text();
    expect(html).toContain('<form method="POST" action="/api/v1/connect-property-monthly-remove"');
  ...
  it('forwards the Access assertion and the period, and redirects to status=ok on success', async () => {
    ...
    expect(url.pathname).toBe('/api/contracts/finance-property-monthly-remove-v1');
