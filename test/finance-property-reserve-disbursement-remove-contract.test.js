...
const PATH = '/api/contracts/finance-property-reserve-disbursement-remove-v1';
...
function insertDisbursement(db, periodKey) {
  db._raw.prepare(
    `INSERT INTO finance_property_reserve_disbursements (property_key, reserve_key, period_key, amount_cents) VALUES ('ivanhoe', 'property_tax', ?, 500000)`
  ).run(periodKey);
}
...
const BODY = { reserve_key: 'property_tax', period_key: '2027' };

describe('POST /api/contracts/finance-property-reserve-disbursement-remove-v1', () => {
  ...
  it('removes a real reserve disbursement for an admin user, always under the fixed ivanhoe property key', async () => {
    const db = makeTestDb();
    insertDisbursement(db, '2027');
    ...
    const row = db._raw.prepare('SELECT * FROM finance_property_reserve_disbursements WHERE property_key=? AND reserve_key=? AND period_key=?').get('ivanhoe', 'property_tax', '2027');
  ...
  it('removing a nonexistent reserve key/period key succeeds as a no-op, same as the legacy DELETE route', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { reserve_key: 'property_tax', period_key: '2099' } });
    expect(res.status).toBe(200);
    expect(await res.json()).toEqual({ ok: true, savedBy: 'root' });
  });

  it('rejects a bad reserve key, the same shape the legacy route\'s own URL regex enforces', async () => {
    const db = makeTestDb();
    insertDisbursement(db, '2027');
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { reserve_key: 'Bad Key!', period_key: '2027' } });
    expect(res.status).toBe(400);
    expect(db._raw.prepare('SELECT * FROM finance_property_reserve_disbursements').all()).toHaveLength(1);
  });

  it('rejects a missing period_key with a 400 and leaves other rows untouched', async () => {
    const db = makeTestDb();
    insertDisbursement(db, '2027');
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { reserve_key: 'property_tax' } });
    expect(res.status).toBe(400);
    expect(db._raw.prepare('SELECT * FROM finance_property_reserve_disbursements').all()).toHaveLength(1);
  });
