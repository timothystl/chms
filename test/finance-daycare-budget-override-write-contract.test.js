...
const PATH = '/api/contracts/finance-daycare-budget-override-write-v1';
...
const BODY = { year: 2025, category: 'Tuition Income', budget: '300000' };

describe('POST /api/contracts/finance-daycare-budget-override-write-v1', () => {
  ...
  it('saves an override for an admin user', async () => {
    const db = makeTestDb();
    ...
    expect(out).toEqual({ ok: true, year: 2025, category: 'Tuition Income', budgetCents: 30000000, savedBy: 'root' });
  ...
  it('an empty budget clears the override instead of inserting $0', async () => {
    ...
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    await post({ env: baseEnv(db), token, body: BODY });
    const res = await post({ env: baseEnv(db), token, body: { ...BODY, budget: '' } });
    expect(res.status).toBe(200);
    expect(db._raw.prepare('SELECT * FROM finance_daycare_entries').all()).toHaveLength(0);
  });

  it('rejects a missing category, the same way the legacy route would', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { year: 2025, budget: '300000' } });
    expect(res.status).toBe(400);
    expect(db._raw.prepare('SELECT * FROM finance_daycare_entries').all()).toHaveLength(0);
  });

  it('rejects a finance-role user -- only admin may edit daycare budget data', async () => {
    const db = makeTestDb();
