...
// (so Distributions renders live and the per-row Remove button has a real row to sit next to).
...
function postForm(env, { accessJwt, body } = {}) {
  const params = new URLSearchParams({ period: '2027-01', ...body });
  return worker.fetch(new Request('https://finance.test/api/v1/connect-property-distribution-remove', {
    method: 'POST',
    headers: {
...
describe('Commercial Property — per-row distribution Remove action and relay route', () => {
  it('rejects the wrong method on the write-only route with an Allow header naming only POST', async () => {
    const res = await worker.fetch(new Request('https://finance.test/api/v1/connect-property-distribution-remove'), baseEnv);
    expect(res.status).toBe(405);
    expect(res.headers.get('allow')).toBe('POST');
  });

  it('does not show the Remove action when the viewer role is not verified', async () => {
    const res = await worker.fetch(new Request('https://finance.test/?section=property&page=distributions'), baseEnv);
    expect(res.status).toBe(200);
    const html = await res.text();
    expect(html).not.toContain('/api/v1/connect-property-distribution-remove');
  });

  it('does not show the Remove action for a finance-role viewer -- admin-only, matching the legacy route', async () => {
  ...
  it('shows a per-row Remove action for a verified admin viewer, on both the Distributions page and Reserve & distribution\'s own sub-table', async () => {
    const env = roleEnv('admin', async () => new Response('not found', { status: 404 }));
    const res = await worker.fetch(new Request('https://finance.test/?section=property&page=distributions', {
      headers: { 'Cf-Access-Jwt-Assertion': 'signed.jwt.here' },
    }), env);
    const html = await res.text();
    expect(html).toContain('<form method="POST" action="/api/v1/connect-property-distribution-remove"');
    expect(html).toContain('name="period" value="2027-01"');

    const res2 = await worker.fetch(new Request('https://finance.test/?section=property&page=reserve-distribution', {
      headers: { 'Cf-Access-Jwt-Assertion': 'signed.jwt.here' },
    }), env);
    const html2 = await res2.text();
    expect(html2).toContain('<form method="POST" action="/api/v1/connect-property-distribution-remove"');
  });
  ...
  it('forwards the Access assertion and the period, and redirects to status=ok on success', async () => {
    let captured;
    ...
    const location = new URL(res.headers.get('location'), 'https://finance.test');
    expect(location.searchParams.get('section')).toBe('property');
    expect(location.searchParams.get('page')).toBe('distributions');
    ...
    expect(url.pathname).toBe('/api/contracts/finance-property-distribution-remove-v1');
    const sentBody = JSON.parse(await captured.text());
    expect(sentBody).toEqual({ period: '2027-01' });
    ...
    expect(html).toContain('Removed in Connect.');
  });

  it('redirects with the refusal reason when Connect declines the removal, and shows it back on the page', async () => {
    ...
    const shown = await worker.fetch(new Request(location.toString(), { headers: { 'Cf-Access-Jwt-Assertion': 'signed.jwt.here' } }), shownEnv);
    const html = await shown.text();
    expect(html).toContain('Not removed: Access denied: editing property financials requires admin access');
  });
