...
const PATH = '/api/contracts/finance-cash-policy-write-v1';
...
const BODY = { policy_floor_months: 6, cash_on_hand_cents: 5000000, cash_account_code: '11027', general_fund_budget_code: '40085' };

describe('POST /api/contracts/finance-cash-policy-write-v1', () => {
  ...
  it('saves the policy for an admin user', async () => {
    const db = makeTestDb();
    ...
    expect(out).toEqual({ ok: true, ...BODY, savedBy: 'root' });

    const row = db._raw.prepare(`SELECT value FROM finance_settings WHERE key='finance_cash_policy'`).get();
    expect(JSON.parse(row.value)).toEqual(BODY);
  });

  it('rejects an out-of-range policy_floor_months, the same way the legacy route would', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { ...BODY, policy_floor_months: 100 } });
    expect(res.status).toBe(400);
    expect(db._raw.prepare(`SELECT * FROM finance_settings WHERE key='finance_cash_policy'`).all()).toHaveLength(0);
  });

  it('rejects a malformed cash_account_code, the same way the legacy route would', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { ...BODY, cash_account_code: 'not an account code!' } });
    expect(res.status).toBe(400);
  });

  it('rejects a finance-role user -- only admin may edit the cash policy', async () => {
    const db = makeTestDb();
