...
describe('Giving analytics pages (Finance v3)', () => {
  ...
  it('lets every totals page switch between all funds, the General Fund, and one fund', async () => {
    ...
    const all = await (await get(env, '&page=year-over-year')).text();
    expect(all).toContain('<a href="/?section=giving-analytics&amp;page=year-over-year" class="is-on" aria-current="true">All funds</a>');
    expect(all).toContain('<option value="8">Building Fund</option>');
    expect(all).toContain('Every gift entered in Connect counts');
    expect(calls.filter((c) => c.path.endsWith('/giving-analytics-v1')).at(-1).query).toBe('');

    const gf = await (await get(env, '&page=year-over-year&fund=general')).text();
    expect(calls.at(-1).query).toBe('?fund=general');
    expect(gf).toContain('class="is-on" aria-current="true">General Fund</a>');
    expect(gf).toContain('General Fund giving by month, 2025 and 2026');
    expect(gf).toContain('General Fund gifts only (3 funds');
    // The sidebar keeps the choice while moving between Giving pages.
    expect(gf).toContain('href="/?section=giving-analytics&amp;page=trends&amp;fund=general"');
    ...
    const trends = await (await get(env, '&page=trends&fund=general')).text();
    expect(trends).toContain('<a href="/?section=giving-analytics&amp;page=trends&amp;fund=8">Building Fund</a>');
    const concentration = await (await worker.fetch(new Request('https://finance.test/?section=charts&page=concentration&fund=general', { headers: { 'Cf-Access-Jwt-Assertion': 'jwt' } }), env)).text();
    expect(calls.at(-1).query).toBe('?fund=general');
    expect(concentration).toContain('Showing General Fund only.');
    expect(concentration).toContain('href="/?section=charts&amp;page=concentration&amp;fund=general" class="is-on"');
  });
