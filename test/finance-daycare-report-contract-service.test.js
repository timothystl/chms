...
const PATH = '/api/contracts/finance-daycare-report-v1';
...
describe('handleContractsServiceApi finance-daycare-report-v1', () => {
  ...
  it('returns a real, valid, empty-categories contract for a fiscal year with nothing on file', async () => {
  ...
  it('reflects real daycare-entry rows for the requested fiscal year only', async () => {
    const db = makeTestDb();
    db._raw.prepare(
    ...
    expect(body.categories[0]).toMatchObject({ category: 'Tuition Income', actualCents: 40000000 });
  });
