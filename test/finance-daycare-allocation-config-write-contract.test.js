...
const PATH = '/api/contracts/finance-daycare-allocation-config-write-v1';
...
const BODY = { utilityPct: 0.3, insurancePct: 0.6 };

describe('POST /api/contracts/finance-daycare-allocation-config-write-v1', () => {
  ...
  it('saves the cost-share config for an admin user', async () => {
    const db = makeTestDb();
    ...
    const row = db._raw.prepare(`SELECT value FROM finance_settings WHERE key='finance_daycare_allocation_config'`).get();
    expect(JSON.parse(row.value)).toEqual(BODY);
  });

  it('rejects a non-numeric percentage, the same way the legacy route would', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { utilityPct: 'not-a-number', insurancePct: 0.5 } });
    expect(res.status).toBe(400);
    expect(db._raw.prepare(`SELECT * FROM finance_settings WHERE key='finance_daycare_allocation_config'`).all()).toHaveLength(0);
  });

  it('rejects a finance-role user -- only admin may edit the daycare cost-share', async () => {
    const db = makeTestDb();
