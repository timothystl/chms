...
const PATH = '/api/contracts/finance-daycare-bulk-write-v1';
...
describe('POST /api/contracts/finance-daycare-bulk-write-v1', () => {
  ...
  it('imports rows for a finance-role user (edit on the finance item by default)', async () => {
    const db = makeTestDb();
    ...
    const res = await post({ env: baseEnv(db), token, body: { rows: ROWS } });
    ...
    const audit = db._raw.prepare(`SELECT action, entity_type, new_value FROM audit_log WHERE action='daycare_bulk_via_finance'`).get();
    expect(audit).toEqual({ action: 'daycare_bulk_via_finance', entity_type: 'finance_daycare_entries', new_value: 'sarah@timothystl.org' });
  });

  it('allows council by default, since compensation defaults to edit and finance/daycare/bulk accepts any of finance/budget/compensation', async () => {
    const db = makeTestDb();
  ...
  it('rejects the whole batch if one row is malformed, saving nothing', async () => {
    ...
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('sarah@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { rows: [
  ...
  it('rejects when the shared X-Contract-Key is wrong, before ever looking at identity', async () => {
    ...
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('sarah@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, contractKey: 'wrong-secret', body: { rows: ROWS } });
  ...
  it('rejects a missing or invalid Access assertion even with a correct contract key', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const noToken = await post({ env: baseEnv(db), body: { rows: ROWS } });
    expect(noToken.status).toBe(401);
    const garbage = await post({ env: baseEnv(db), token: 'not-a-jwt', body: { rows: ROWS } });
  ...
  it('returns 503 when Connect has not been configured with the Access team/audience yet', async () => {
    ...
    const res = await post({ env, token, body: { rows: ROWS } });
