...
// (so Distributions renders live, the same two calls a real page load makes -- otherwise the page
// falls back to a synthetic read that needs a FINANCE_DB binding this test env doesn't carry).
...
function postForm(env, { accessJwt, body } = {}) {
  const params = new URLSearchParams({
    period: '2027-01', amount: '5000',
    ...body,
  });
  return worker.fetch(new Request('https://finance.test/api/v1/connect-property-distribution-write', {
    method: 'POST',
    headers: {
...
describe('Commercial Property — distribution entry form and relay route', () => {
  it('rejects the wrong method on the write-only route with an Allow header naming only POST', async () => {
    const res = await worker.fetch(new Request('https://finance.test/api/v1/connect-property-distribution-write'), baseEnv);
  ...
  it('does not show the entry form when the viewer role is not verified', async () => {
    ...
    expect(html).not.toContain('/api/v1/connect-property-distribution-write');
  });
  ...
  it('shows the entry form for a verified admin viewer', async () => {
    ...
    const html = await res.text();
    expect(html).toContain('<form method="POST" action="/api/v1/connect-property-distribution-write">');
  ...
  it('forwards the Access assertion and form fields, and redirects to status=ok on success', async () => {
    ...
    expect(url.pathname).toBe('/api/contracts/finance-property-distribution-write-v1');
    const sentBody = JSON.parse(await captured.text());
    expect(sentBody).toEqual({ period: '2027-01', amount: '5000' });
