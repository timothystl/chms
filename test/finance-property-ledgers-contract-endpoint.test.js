...
const SEG = 'contracts/finance-property-ledgers-v1';
...
describe('contracts/finance-property-ledgers-v1 access gate', () => {
...
describe('contracts/finance-property-ledgers-v1 real handler', () => {
  ...
  it('reflects real capital/repairs rows for the default (ivanhoe) property', async () => {
    const db = makeTestDb();
    ...
    db._raw.prepare(`INSERT INTO finance_property_repairs (property_key,entry_date,category,amount_cents) VALUES (?,?,?,?)`)
      .run('ivanhoe', '2026-05', 'HVAC', null);
    const env = { DB: db };
    const url = new URL(`https://connect.example/admin/api/${SEG}`);
    const req = { json: async () => ({}), headers: { get: () => null } };
    const res = await handleChmsApi(req, env, url, 'GET', SEG, 'admin');
    ...
    expect(body.repairs[0].amountCents).toBeNull();
  });
