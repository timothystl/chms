...
// Answers both staff-role-v1 (so canManageDaycareAllocation resolves) and finance-daycare-report-v1
// (so Shared costs renders live) -- the same two calls a real page load makes.
...
function postForm(env, { accessJwt, body } = {}) {
  const params = new URLSearchParams({
    utility_pct: '0.3', insurance_pct: '0.6',
    ...body,
  });
  return worker.fetch(new Request('https://finance.test/api/v1/connect-daycare-allocation-config-write', {
    method: 'POST',
    headers: {
...
describe('Daycare Report — cost-share config form and relay route', () => {
  it('rejects the wrong method on the write-only route with an Allow header naming only POST', async () => {
    const res = await worker.fetch(new Request('https://finance.test/api/v1/connect-daycare-allocation-config-write'), baseEnv);
    expect(res.status).toBe(405);
    expect(res.headers.get('allow')).toBe('POST');
  });

  it('does not show the config form when the viewer role is not verified', async () => {
    const res = await worker.fetch(new Request('https://finance.test/?section=daycare&page=shared-costs'), baseEnv);
    expect(res.status).toBe(200);
    const html = await res.text();
    expect(html).not.toContain('/api/v1/connect-daycare-allocation-config-write');
  });

  it('does not show the config form for a finance-role viewer -- admin-only, matching the legacy route', async () => {
    const env = roleEnv('finance', async () => new Response('not found', { status: 404 }));
    const res = await worker.fetch(new Request('https://finance.test/?section=daycare&page=shared-costs', {
  ...
  it('shows the config form for a verified admin viewer', async () => {
    const env = roleEnv('admin', async () => new Response('not found', { status: 404 }));
    const res = await worker.fetch(new Request('https://finance.test/?section=daycare&page=shared-costs', {
      headers: { 'Cf-Access-Jwt-Assertion': 'signed.jwt.here' },
    }), env);
    const html = await res.text();
    expect(html).toContain('<form method="POST" action="/api/v1/connect-daycare-allocation-config-write">');
    expect(html).toContain('name="utility_pct"');
    expect(html).toContain('name="insurance_pct"');
  });
  ...
  it('forwards the Access assertion and form fields, and redirects to status=ok on success', async () => {
    ...
    const location = new URL(res.headers.get('location'), 'https://finance.test');
    expect(location.searchParams.get('section')).toBe('daycare');
    expect(location.searchParams.get('page')).toBe('shared-costs');
    ...
    expect(url.pathname).toBe('/api/contracts/finance-daycare-allocation-config-write-v1');
    const sentBody = JSON.parse(await captured.text());
    expect(sentBody).toEqual({ utilityPct: '0.3', insurancePct: '0.6' });
  ...
  it('redirects with the refusal reason when Connect declines the entry, and shows it back on the page', async () => {
    const env = liveEnv(async () => new Response(JSON.stringify({ error: 'Access denied: editing the daycare cost-share requires admin access' }), { status: 403 }));
    const res = await postForm(env, { accessJwt: 'signed.jwt.here' });
    const location = new URL(res.headers.get('location'), 'https://finance.test');
    expect(location.searchParams.get('status')).toBe('error');
    expect(location.searchParams.get('reason')).toBe('http_error');
    expect(location.searchParams.get('message')).toBe('Access denied: editing the daycare cost-share requires admin access');

    const shownEnv = roleEnv('admin', async () => new Response('not found', { status: 404 }));
    const shown = await worker.fetch(new Request(location.toString(), { headers: { 'Cf-Access-Jwt-Assertion': 'signed.jwt.here' } }), shownEnv);
    const html = await shown.text();
    expect(html).toContain('Not saved: Access denied: editing the daycare cost-share requires admin access');
  });
