...
import { fetchLiveFinancePropertyReserves } from '../apps/finance/finance-property-reserves-client.js';
...
const VALID = {
  ...
  reserves: [{ reserveKey: 'property_tax', reportMonth: '2026-05', taxYear: 2026, targetEstimateCents: 1140000, reserveBeforeCents: 380000, contributionCents: 95000, reserveAfterCents: 475000, fundedPct: (475000 / 1140000) * 100, note: '' }],
...
describe('fetchLiveFinancePropertyReserves', () => {
  it('is not_configured when the service binding is missing', async () => {
    const result = await fetchLiveFinancePropertyReserves({ FINANCE_CONTRACT_API_KEY: 'x' });
    expect(result).toEqual({ ok: false, reason: 'not_configured' });
  });

  it('is not_configured when the shared secret is missing', async () => {
    const result = await fetchLiveFinancePropertyReserves({ CONNECT_SERVICE: { fetch: async () => new Response('{}') } });
    expect(result).toEqual({ ok: false, reason: 'not_configured' });
  });
  ...
  it('sends the shared secret header and default property key, and accepts a valid response', async () => {
    ...
    expect(result.reserves.reserves).toHaveLength(1);
    expect(capturedRequest.headers.get('X-Contract-Key')).toBe('test-secret');
    const url = new URL(capturedRequest.url);
    expect(url.pathname).toBe('/api/contracts/finance-property-reserves-v1');
  ...
  it('requests an explicitly passed property key', async () => {
    let capturedRequest;
    const env = envWith(async (req) => { capturedRequest = req; return new Response(JSON.stringify(VALID), { status: 200 }); });
    await fetchLiveFinancePropertyReserves(env, 'other-property');
  ...
  it('fails closed on a well-formed but contract-invalid payload -- never trusts the wire blindly', async () => {
    const env = envWith(async () => new Response(JSON.stringify({ contract: 'connect.finance-property-reserves.v1' }), { status: 200 }));
    const result = await fetchLiveFinancePropertyReserves(env);
