...
function fakeXlsxFile(content = 'PK\x03\x04fake-activity-xlsx-bytes') {
  return new File([content], 'activity.xlsx', { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
}

function postFile(env, { accessJwt, file } = {}) {
  const form = new FormData();
  if (file !== null) form.set('file', file === undefined ? fakeXlsxFile() : file);
  return worker.fetch(new Request('https://finance.test/api/v1/connect-church-activity-xlsx-import-write', {
    method: 'POST',
...
describe('Church Report — Statement of Activity multi-year .xlsx import form and relay route', () => {
  it('rejects the wrong method on the write-only route with an Allow header naming only POST', async () => {
    const res = await worker.fetch(new Request('https://finance.test/api/v1/connect-church-activity-xlsx-import-write'), baseEnv);
  ...
  it('shows the import form for a verified finance-role viewer -- not admin-only, unlike the annual Budget vs. Actuals import', async () => {
    ...
    expect(overviewHtml).not.toContain('/api/v1/connect-church-activity-xlsx-import-write');
  });

  it('shows the import form for a council viewer too -- the looser any-verified-role gate, unlike the admin-only Budget vs. Actuals form', async () => {
  ...
  it('base64-encodes the uploaded bytes, forwards the Access assertion, and redirects to status=ok on success', async () => {
    ...
    const content = 'PK\x03\x04fake-activity-xlsx-bytes';
    ...
    expect(url.pathname).toBe('/api/contracts/finance-church-activity-xlsx-import-v1');
