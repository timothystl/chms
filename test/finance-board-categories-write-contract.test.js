...
const PATH = '/api/contracts/finance-board-categories-write-v1';
...
describe('POST /api/contracts/finance-board-categories-write-v1', () => {
  ...
  it('merges a real expense-category assignment for an admin, into the real shared store', async () => {
    const db = makeTestDb();
    ...
    const res = await post({ env: baseEnv(db), token, body: { expense: { 'Expenses:60000 Programs': 'programs' } } });
    expect(res.status).toBe(200);
    const out = await res.json();
    expect(out.ok).toBe(true);
    expect(out.expense).toEqual({ 'Expenses:60000 Programs': 'programs' });
    expect(out.savedBy).toBe('root');

    const row = db._raw.prepare(`SELECT value FROM finance_settings WHERE key='finance_planning_board_categories'`).get();
    expect(JSON.parse(row.value).expense).toEqual({ 'Expenses:60000 Programs': 'programs' });
  });

  it('merges without clobbering an unrelated existing assignment', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    db._raw.prepare(`INSERT INTO finance_settings (key,value) VALUES ('finance_planning_board_categories', ?)`)
      .run(JSON.stringify({ revenue: { 'Income:40000 Contributions': 'donor' }, expense: {}, revenueLabels: {}, expenseLabels: {}, donorWrapperLabel: '', accountLabels: {} }));
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));

    const res = await post({ env: baseEnv(db), token, body: { expense: { 'Expenses:60000 Programs': 'programs' } } });
    const out = await res.json();
    expect(out.revenue).toEqual({ 'Income:40000 Contributions': 'donor' });
    expect(out.expense).toEqual({ 'Expenses:60000 Programs': 'programs' });
  });

  it('rejects an invalid expense category key, the same way the legacy route would', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { expense: { 'Expenses:x': 'not_a_real_key' } } });
    expect(res.status).toBe(400);
  });

  it('rejects a council user -- board-category assignment is admin-only, unlike Budget override-bulk', async () => {
    const db = makeTestDb();
    ...
    const res = await post({ env: baseEnv(db), token, body: { expense: { 'Expenses:x': 'programs' } } });
  ...
  it('rejects when the shared X-Contract-Key is wrong, before ever looking at identity', async () => {
    ...
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, contractKey: 'wrong-secret', body: {} });
  ...
  it('clears an assignment back to default when the sent value is empty', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    db._raw.prepare(`INSERT INTO finance_settings (key,value) VALUES ('finance_planning_board_categories', ?)`)
      .run(JSON.stringify({ revenue: {}, expense: { 'Expenses:60000 Programs': 'programs' }, revenueLabels: {}, expenseLabels: {}, donorWrapperLabel: '', accountLabels: {} }));
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { expense: { 'Expenses:60000 Programs': '' } } });
    const out = await res.json();
    expect(out.expense).toEqual({});
  });
});
