...
// (so Reserve & distribution renders live and the per-row Remove button has a real row to sit next to).
...
function postForm(env, { accessJwt, body } = {}) {
  const params = new URLSearchParams({ reserve_key: 'property_tax', report_month: '2027-01', ...body });
  return worker.fetch(new Request('https://finance.test/api/v1/connect-property-reserve-monthly-remove', {
    method: 'POST',
    headers: {
...
describe('Commercial Property — per-row reserve-schedule Remove action and relay route', () => {
  it('rejects the wrong method on the write-only route with an Allow header naming only POST', async () => {
    const res = await worker.fetch(new Request('https://finance.test/api/v1/connect-property-reserve-monthly-remove'), baseEnv);
  ...
  it('does not show the Remove action when the viewer role is not verified', async () => {
    ...
    expect(html).not.toContain('/api/v1/connect-property-reserve-monthly-remove');
  });
  ...
  it('shows a per-row Remove action (with the reserve key carried as a hidden field) for a verified admin viewer', async () => {
    const env = roleEnv('admin', async () => new Response('not found', { status: 404 }));
    const res = await worker.fetch(new Request('https://finance.test/?section=property&page=reserve-distribution', {
      headers: { 'Cf-Access-Jwt-Assertion': 'signed.jwt.here' },
    }), env);
    const html = await res.text();
    expect(html).toContain('<form method="POST" action="/api/v1/connect-property-reserve-monthly-remove"');
    expect(html).toContain('name="reserve_key" value="property_tax"');
    expect(html).toContain('name="report_month" value="2027-01"');
  });
  ...
  it('forwards the Access assertion and the reserve key/report month, and redirects to status=ok on success', async () => {
    let captured;
    ...
    expect(url.pathname).toBe('/api/contracts/finance-property-reserve-monthly-remove-v1');
    const sentBody = JSON.parse(await captured.text());
    expect(sentBody).toEqual({ reserve_key: 'property_tax', report_month: '2027-01' });
