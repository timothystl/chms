...
const PATH = '/api/contracts/finance-daycare-entry-edit-v1';
...
const ORIGINAL = { period: '2027', category: 'Tuition Income', entry_type: 'actual', amount_cents: 500000, notes: 'first' };

describe('POST /api/contracts/finance-daycare-entry-edit-v1', () => {
  ...
  it('edits a real entry for a finance-role user, only touching the fields sent', async () => {
    const db = makeTestDb();
    ...
    const res = await post({ env: baseEnv(db), token, body: { id, amount_cents: 750000 } });
    ...
    expect(row).toEqual({ ...ORIGINAL, amount_cents: 750000 });

    const audit = db._raw.prepare(`SELECT action, entity_type, entity_id, new_value FROM audit_log WHERE action='daycare_entry_edit_via_finance'`).get();
    expect(audit).toEqual({ action: 'daycare_entry_edit_via_finance', entity_type: 'finance_daycare_entries', entity_id: id, new_value: 'sarah@timothystl.org' });
  });
  ...
  it('allows council by default, since compensation defaults to edit and the looser gate accepts any of finance/budget/compensation', async () => {
    ...
    const res = await post({ env: baseEnv(db), token, body: { id, category: 'Payroll' } });
    expect(res.status).toBe(200);
  });
  ...
  it('allows admin unconditionally', async () => {
    ...
    const res = await post({ env: baseEnv(db), token, body: { id, notes: 'corrected' } });
    expect(res.status).toBe(200);
  });
  ...
  it('rejects a staff-role user -- finance/budget/compensation all default to none for staff', async () => {
    ...
    const res = await post({ env: baseEnv(db), token, body: { id, notes: 'nope' } });
    expect(res.status).toBe(403);
    const row = db._raw.prepare('SELECT notes FROM finance_daycare_entries WHERE id=?').get(id);
    expect(row.notes).toBe('first');
  });

  it('returns 404 for an id that does not exist, matching the legacy PUT route exactly', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('sarah@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { id: 999999, notes: 'x' } });
  ...
  it('rejects an id that is not a positive integer', async () => {
    ...
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('sarah@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { id: 'not-an-id', notes: 'x' } });
    expect(res.status).toBe(400);
  });

  it('rejects an invalid period, saving nothing, matching the legacy PUT route validation', async () => {
    const db = makeTestDb();
    const id = insertEntry(db, ORIGINAL);
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('sarah@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { id, period: 'not-a-period' } });
    expect(res.status).toBe(400);
  ...
  it('rejects when the shared X-Contract-Key is wrong, before ever looking at identity', async () => {
    ...
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('sarah@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, contractKey: 'wrong-secret', body: { id, notes: 'x' } });
  ...
  it('rejects a missing or invalid Access assertion even with a correct contract key', async () => {
    ...
    const id = insertEntry(db, ORIGINAL);
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const noToken = await post({ env: baseEnv(db), body: { id, notes: 'x' } });
    expect(noToken.status).toBe(401);
    const garbage = await post({ env: baseEnv(db), token: 'not-a-jwt', body: { id, notes: 'x' } });
  ...
  it('rejects an identity Access verifies but that has no matching Connect account', async () => {
    ...
    const res = await post({ env: baseEnv(db), token, body: { id, notes: 'x' } });
    expect(res.status).toBe(403);
  });
  ...
  it('returns 503 when Connect has not been configured with the Access team/audience yet', async () => {
    ...
    const res = await post({ env, token, body: { id, notes: 'x' } });
