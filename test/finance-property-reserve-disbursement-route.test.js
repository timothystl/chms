...
function postForm(env, { accessJwt, body } = {}) {
  const params = new URLSearchParams({
    reserve_key: 'property_tax', period_key: '2025', amount: '11349.64', paid_via_report_month: '2025-11',
    ...body,
  });
  return worker.fetch(new Request('https://finance.test/api/v1/connect-property-reserve-disbursement-write', {
    method: 'POST',
    headers: {
...
describe('Commercial Property — reserve disbursement entry form and relay route', () => {
  it('rejects the wrong method on the write-only route with an Allow header naming only POST', async () => {
    const res = await worker.fetch(new Request('https://finance.test/api/v1/connect-property-reserve-disbursement-write'), baseEnv);
  ...
  it('does not show the entry form when the viewer role is not verified', async () => {
    ...
    expect(html).not.toContain('/api/v1/connect-property-reserve-disbursement-write');
  });
  ...
  it('shows the entry form for a verified admin viewer, alongside the reserve schedule form', async () => {
    ...
    expect(html).toContain('name="period_key"');
  ...
  it('forwards the Access assertion and form fields, and redirects to status=ok on success', async () => {
    ...
    expect(url.pathname).toBe('/api/contracts/finance-property-reserve-disbursement-write-v1');
    const sentBody = JSON.parse(await captured.text());
    expect(sentBody).toEqual({
      reserve_key: 'property_tax', period_key: '2025', amount: '11349.64',
      paid_via_report_month: '2025-11', note: '',
    });
