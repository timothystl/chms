...
const PATH = '/api/contracts/finance-church-budget-multi-year-xlsx-import-v1';
...
// A "Budget by Year" multi-year export: one column per fiscal year (bare "2026"/"2027"
// headers), Budget only. 'Income'/'Expenses' header rows each have children, so they also
// produce a $0 flat row per year -- four accounts x two years = 8 total rows.
function buildBudgetByYearXlsx({ sheetName = 'Sheet1' } = {}) {
...
${xmlRow(1, [cellStr('A1', 'Budget by Year')])}
...
describe('POST /api/contracts/finance-church-budget-multi-year-xlsx-import-v1', () => {
  ...
  it('previews without writing, then commits only the selected budget rows', async () => {
    ...
    const commitRes = await post({ env: baseEnv(db), token, path: '/api/contracts/finance-church-budget-multi-year-xlsx-commit-v1', body: { years: preview.years, rows: preview.rows.slice(0, 1) } });
  ...
  it('saves real rows for a finance-role user, tagged source=import_activity, budget only', async () => {
    ...
    expect(util2027.own_budget_cents).toBe(22000);
    expect(util2027.own_actual_cents).toBe(0);

    const logRow = db._raw.prepare(`SELECT note FROM finance_import_log WHERE importer_key='church_budget_multi'`).get();
