...
const PATH = '/api/contracts/finance-church-actual-override-v1';
...
function insertChurchEntry(db, { fiscalYear, category, source, actualCents = 0 }) {
  db._raw.prepare(
    `INSERT INTO finance_church_entries
     ...
     VALUES (?,0,'Expenses',?,?,0,0,?,NULL,?,datetime('now'))`
  ).run(fiscalYear, category, category, actualCents, source);
}
...
const ROW = { category: 'Expenses:Utilities', classification: 'Expenses', account_name: 'Utilities', amount: '5000' };

describe('POST /api/contracts/finance-church-actual-override-v1', () => {
  ...
  it('saves a real correction for an admin user, taking precedence in resolveChurchYearPrecedence', async () => {
    const db = makeTestDb();
    ...
    const res = await post({ env: baseEnv(db), token, body: { year: 2027, rows: [ROW] } });
    expect(res.status).toBe(200);
    const out = await res.json();
    expect(out).toEqual({ ok: true, year: 2027, saved: 1, savedBy: 'root' });

    const row = db._raw.prepare(
      `SELECT * FROM finance_church_entries WHERE fiscal_year=? AND category_path=? AND source='manual_actual_override'`
    ).get(2027, 'Expenses:Utilities');
    expect(row.own_actual_cents).toBe(500000);
    expect(row.notes).toBe('Manually corrected');
  });

  it('clears a prior correction when amount is blank, deleting only the override row', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    insertChurchEntry(db, { fiscalYear: 2027, category: 'Expenses:Utilities', source: 'qbo_sync', actualCents: 100000 });
    insertChurchEntry(db, { fiscalYear: 2027, category: 'Expenses:Utilities', source: 'manual_actual_override', actualCents: 500000 });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));

    const res = await post({ env: baseEnv(db), token, body: { year: 2027, rows: [{ ...ROW, amount: '' }] } });
    expect(res.status).toBe(200);

    const rows = db._raw.prepare(`SELECT source FROM finance_church_entries WHERE fiscal_year=2027 AND category_path='Expenses:Utilities'`).all();
    expect(rows).toEqual([{ source: 'qbo_sync' }]);
  });

  it('rejects a finance-role user -- only admin may correct an actual, same as the legacy route', async () => {
    const db = makeTestDb();
  ...
  it('rejects a council user too -- unlike Budget override-bulk, this is admin-only', async () => {
    const db = makeTestDb();
  ...
  it('rejects when the shared X-Contract-Key is wrong, before ever looking at identity', async () => {
    ...
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, contractKey: 'wrong-secret', body: { year: 2027, rows: [ROW] } });
  ...
  it('rejects a missing or invalid Access assertion even with a correct contract key', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const noToken = await post({ env: baseEnv(db), body: { year: 2027, rows: [ROW] } });
    expect(noToken.status).toBe(401);
    const garbage = await post({ env: baseEnv(db), token: 'not-a-jwt', body: { year: 2027, rows: [ROW] } });
  ...
  it('returns 503 when Connect has not been configured with the Access team/audience yet', async () => {
    ...
    const res = await post({ env, token, body: { year: 2027, rows: [ROW] } });
    expect(res.status).toBe(503);
  });

  it('still applies the same validation as the human route (missing category -> 400, saving nothing)', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { year: 2027, rows: [{ amount: '100' }] } });
