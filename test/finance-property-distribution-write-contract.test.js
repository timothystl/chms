...
const PATH = '/api/contracts/finance-property-distribution-write-v1';
...
const ROW = { period: '2027-01', amount: '5000' };

describe('POST /api/contracts/finance-property-distribution-write-v1', () => {
  ...
  it('saves a real distribution for an admin user, always under the fixed ivanhoe property key', async () => {
    const db = makeTestDb();
  ...
  it('ignores a client-supplied property_key -- the property is always ivanhoe on Connect\'s side', async () => {
    ...
    expect(res.status).toBe(200);
    expect(db._raw.prepare(`SELECT DISTINCT property_key FROM finance_property_distributions`).all()).toEqual([{ property_key: 'ivanhoe' }]);
  });

  it('re-saving the same period upserts rather than duplicating, same as the legacy route', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    await post({ env: baseEnv(db), token, body: ROW });
    await post({ env: baseEnv(db), token, body: { ...ROW, amount: '6250.50' } });
    const rows = db._raw.prepare('SELECT * FROM finance_property_distributions').all();
    expect(rows).toHaveLength(1);
    expect(rows[0].amount_cents).toBe(625050);
  });

  it('places no sign restriction on amount -- a negative correction is allowed, same as the legacy route', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { period: '2027-02', amount: '-100' } });
    expect(res.status).toBe(200);
    const row = db._raw.prepare('SELECT amount_cents FROM finance_property_distributions WHERE period=?').get('2027-02');
    expect(row.amount_cents).toBe(-10000);
  });

  it('rejects a bad period, the same way the legacy route would', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { ...ROW, period: '2027' } });
  ...
  it('rejects an invalid amount, the same way the legacy route would', async () => {
    ...
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { ...ROW, amount: 'not-a-number' } });
    expect(res.status).toBe(400);
    expect(db._raw.prepare('SELECT * FROM finance_property_distributions').all()).toHaveLength(0);
  });
