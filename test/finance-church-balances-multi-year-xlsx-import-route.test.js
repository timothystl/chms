...
// Same live trend fixture shape as test/finance-alpha-shell.test.js's own Balance Sheet trend
// mock -- the 'multi-year' page's synthetic fallback reads Finance's own FINANCE_DB, which these
// route tests never provision, so the live trend contract has to be answered (not just
// staff-role-v1) or the page renders its generic "Data unavailable" placeholder instead of the
// form under test.
const VALID_LIVE_BALANCE_TREND = {
  ...
  years: [
    {
      fiscalYear: 2027, asOfDate: 'December 31, 2027', assetsCents: 500000, liabilitiesCents: 100000,
      equityCents: 400000, netAssetsCents: 400000, balancedCents: 0,
    },
  ],
...
function fakeXlsxFile(content = 'PK\x03\x04fake-balance-multi-year-xlsx-bytes') {
  return new File([content], 'balances-multi-year.xlsx', { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
}

function postFile(env, { accessJwt, file } = {}) {
  const form = new FormData();
  if (file !== null) form.set('file', file === undefined ? fakeXlsxFile() : file);
  return worker.fetch(new Request('https://finance.test/api/v1/connect-church-balances-multi-year-xlsx-import-write', {
    method: 'POST',
...
describe('Balance Sheet — Statement of Financial Position multi-year .xlsx import form and relay route', () => {
  it('rejects the wrong method on the write-only route with an Allow header naming only POST', async () => {
    const res = await worker.fetch(new Request('https://finance.test/api/v1/connect-church-balances-multi-year-xlsx-import-write'), baseEnv);
    expect(res.status).toBe(405);
    expect(res.headers.get('allow')).toBe('POST');
  });

  it('does not show the import form when the viewer role is not verified', async () => {
    const res = await worker.fetch(new Request('https://finance.test/?section=balance&page=multi-year'), baseEnv);
  ...
  it('shows the import form for a verified finance-role viewer -- not admin-only, unlike the single-snapshot Statement of Financial Position import', async () => {
    ...
    const positionRes = await worker.fetch(new Request('https://finance.test/?section=balance&page=position', {
      headers: { 'Cf-Access-Jwt-Assertion': 'signed.jwt.here' },
    }), env);
    const positionHtml = await positionRes.text();
  ...
  it('redirects to a no_file error when no file was attached', async () => {
    ...
    expect(location.searchParams.get('page')).toBe('multi-year');
  ...
  it('base64-encodes the uploaded bytes, forwards the Access assertion, and redirects to status=ok on success', async () => {
    ...
    const env = liveEnv(async (req) => {
      captured = req;
      return new Response(JSON.stringify({ ok: true, years: [2026, 2027], basis: null, imported: 12, savedBy: 'andrew' }), { status: 200 });
    });
    const content = 'PK\x03\x04fake-balance-multi-year-xlsx-bytes';
    ...
    expect(url.pathname).toBe('/api/contracts/finance-church-balances-multi-year-xlsx-import-v1');
