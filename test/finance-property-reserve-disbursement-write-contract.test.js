...
const PATH = '/api/contracts/finance-property-reserve-disbursement-write-v1';
...
const ROW = { reserve_key: 'property_tax', period_key: '2025', amount: '11349.64', paid_via_report_month: '2025-11', note: 'annual tax bill' };

describe('POST /api/contracts/finance-property-reserve-disbursement-write-v1', () => {
  ...
  it('saves a real disbursement for an admin user, always under the fixed ivanhoe property key', async () => {
    const db = makeTestDb();
    ...
    const row = db._raw.prepare('SELECT * FROM finance_property_reserve_disbursements WHERE property_key=? AND reserve_key=? AND period_key=?').get('ivanhoe', 'property_tax', '2025');
    expect(row.amount_cents).toBe(1134964);
    expect(row.paid_via_report_month).toBe('2025-11');
    expect(row.note).toBe('annual tax bill');
  });
  ...
  it('ignores a client-supplied property_key -- the property is always ivanhoe on Connect\'s side', async () => {
    ...
    expect(res.status).toBe(200);
    expect(db._raw.prepare(`SELECT DISTINCT property_key FROM finance_property_reserve_disbursements`).all()).toEqual([{ property_key: 'ivanhoe' }]);
  });

  it('allows an omitted amount -- period_key alone marks a not-yet-paid disbursement, same as the legacy route', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { reserve_key: 'property_tax', period_key: '2025', note: 'Not yet paid.' } });
    expect(res.status).toBe(200);
    const row = db._raw.prepare('SELECT amount_cents FROM finance_property_reserve_disbursements WHERE period_key=?').get('2025');
    expect(row.amount_cents).toBeNull();
  });

  it('re-saving the same reserve key and period key upserts rather than duplicating, same as the legacy route', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    await post({ env: baseEnv(db), token, body: ROW });
    await post({ env: baseEnv(db), token, body: { ...ROW, amount: '11400', paid_via_report_month: '2025-12', note: 'revised' } });
    const rows = db._raw.prepare('SELECT * FROM finance_property_reserve_disbursements').all();
    expect(rows).toHaveLength(1);
    expect(rows[0].amount_cents).toBe(1140000);
    expect(rows[0].note).toBe('revised');
  });

  it('never checks a disbursement against the reserve schedule\'s running balance, same as the legacy route', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { reserve_key: 'property_tax', period_key: '2026', amount: '10000000' } });
    expect(res.status).toBe(200);
  });

  it('rejects a missing period_key, the same way the legacy route would', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { reserve_key: 'property_tax', amount: '100' } });
    expect(res.status).toBe(400);
    expect(db._raw.prepare('SELECT * FROM finance_property_reserve_disbursements').all()).toHaveLength(0);
  });
  ...
  it('rejects a reserve key that does not match [a-z_]+', async () => {
    ...
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { ...ROW, reserve_key: 'PropertyTax' } });
    expect(res.status).toBe(400);
    expect(db._raw.prepare('SELECT * FROM finance_property_reserve_disbursements').all()).toHaveLength(0);
  });
