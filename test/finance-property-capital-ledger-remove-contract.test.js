...
const PATH = '/api/contracts/finance-property-capital-ledger-remove-v1';
...
function insertCapitalLedgerEntry(db) {
  const r = db._raw.prepare(
    `INSERT INTO finance_property_capital_ledger (property_key, entry_date, amount_cents, payee, description) VALUES ('ivanhoe', '2027-01-15', 500000, 'Acme Roofing', 'Roof replacement')`
  ).run();
...
describe('POST /api/contracts/finance-property-capital-ledger-remove-v1', () => {
  ...
  it('removes a real capital-ledger entry for an admin user, always under the fixed ivanhoe property key', async () => {
    const db = makeTestDb();
    const id = insertCapitalLedgerEntry(db);
    ...
    const row = db._raw.prepare('SELECT * FROM finance_property_capital_ledger WHERE property_key=? AND id=?').get('ivanhoe', id);
  ...
  it('ignores a client-supplied property_key -- the property is always ivanhoe on Connect\'s side', async () => {
    ...
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { id, property_key: 'some-other-property' } });
    expect(res.status).toBe(200);
    expect(db._raw.prepare('SELECT * FROM finance_property_capital_ledger').all()).toHaveLength(0);
  });

  it('removing a nonexistent id succeeds as a no-op, same as the legacy DELETE route', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
  ...
  it('rejects a non-numeric id with a 400 and leaves other rows untouched', async () => {
    const db = makeTestDb();
    insertCapitalLedgerEntry(db);
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { id: 'not-a-number' } });
    expect(res.status).toBe(400);
    expect(db._raw.prepare('SELECT * FROM finance_property_capital_ledger').all()).toHaveLength(1);
  });
