...
import { fetchLiveFinancePropertyOperating } from '../apps/finance/finance-property-operating-client.js';
...
const VALID = {
  ...
  periods: [
    { period: '2026-06', occupancyPct: 1, totalRevenueCents: 100, totalExpensesCents: 40, netIncomeCents: 60, netOperatingIncomeCents: null, availableForDistributionCents: null, reserveBalanceCents: null, loanPaymentCents: null, interestExpenseCents: null, sourceReport: '' },
  ],
  annualSummary: [],
};
...
describe('fetchLiveFinancePropertyOperating', () => {
  it('is not_configured when the service binding is missing', async () => {
    const result = await fetchLiveFinancePropertyOperating({ FINANCE_CONTRACT_API_KEY: 'x' });
    expect(result).toEqual({ ok: false, reason: 'not_configured' });
  });

  it('is not_configured when the shared secret is missing', async () => {
    const result = await fetchLiveFinancePropertyOperating({ CONNECT_SERVICE: { fetch: async () => new Response('{}') } });
    expect(result).toEqual({ ok: false, reason: 'not_configured' });
  });
  ...
  it('sends the shared secret header and default property key, and accepts a valid response', async () => {
    ...
    expect(result.operating.periods).toHaveLength(1);
    expect(capturedRequest.headers.get('X-Contract-Key')).toBe('test-secret');
    const url = new URL(capturedRequest.url);
    expect(url.pathname).toBe('/api/contracts/finance-property-operating-v1');
  ...
  it('requests an explicitly passed property key', async () => {
    let capturedRequest;
    const env = envWith(async (req) => { capturedRequest = req; return new Response(JSON.stringify(VALID), { status: 200 }); });
    await fetchLiveFinancePropertyOperating(env, 'other-property');
  ...
  it('fails closed on a well-formed but contract-invalid payload -- never trusts the wire blindly', async () => {
    const env = envWith(async () => new Response(JSON.stringify({ contract: 'connect.finance-property-operating.v1' }), { status: 200 }));
    const result = await fetchLiveFinancePropertyOperating(env);
