...
function postForm(env, { accessJwt, body } = {}) {
  const params = new URLSearchParams({
    period: '2027', category: 'Tuition Income', entry_type: 'actual', amount: '5000', notes: '',
    ...body,
  });
  return worker.fetch(new Request('https://finance.test/api/v1/connect-daycare-entry', {
    method: 'POST',
    headers: {
...
describe('Daycare Report — record-an-entry form and relay route', () => {
  it('rejects the wrong method on the write-only route with an Allow header naming only POST', async () => {
    const res = await worker.fetch(new Request('https://finance.test/api/v1/connect-daycare-entry'), baseEnv);
    expect(res.status).toBe(405);
    expect(res.headers.get('allow')).toBe('POST');
  });

  it('does not show the entry form when the viewer role is not verified (fail-closed on the write UI)', async () => {
    const res = await worker.fetch(new Request('https://finance.test/?section=daycare&page=actuals'), baseEnv);
    expect(res.status).toBe(200);
    const html = await res.text();
    expect(html).not.toContain('/api/v1/connect-daycare-entry');
  });

  it('shows the entry form for any verified role that can reach the Daycare section, e.g. finance', async () => {
    ...
    const html = await res.text();
    expect(html).toContain('<form method="POST" action="/api/v1/connect-daycare-entry">');
  ...
  it('forwards the Access assertion and converts dollars to cents, redirecting to status=ok on success', async () => {
    let captured;
    const env = liveEnv(async (req) => {
      captured = req;
      return new Response(JSON.stringify({ ok: true, id: 42, savedBy: 'andrew' }), { status: 200 });
    });
    const res = await postForm(env, { accessJwt: 'signed.jwt.here', body: { amount: '75.50' } });
    expect(res.status).toBe(303);
    const location = new URL(res.headers.get('location'), 'https://finance.test');
    expect(location.searchParams.get('section')).toBe('daycare');
    ...
    expect(url.pathname).toBe('/api/contracts/finance-daycare-entry-v1');
    const sentBody = JSON.parse(await captured.text());
    expect(sentBody).toEqual({ period: '2027', category: 'Tuition Income', entry_type: 'actual', amount_cents: 7550, notes: '' });
  ...
  it('redirects with the refusal reason when Connect declines the entry, and shows it back on the page', async () => {
    ...
    const shown = await worker.fetch(new Request(location.toString(), { headers: { 'Cf-Access-Jwt-Assertion': 'signed.jwt.here' } }), shownEnv);
    const html = await shown.text();
    expect(html).toContain('Not saved: Access denied');
  });
