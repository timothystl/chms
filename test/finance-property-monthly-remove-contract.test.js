...
const PATH = '/api/contracts/finance-property-monthly-remove-v1';
...
function insertMonthly(db, period) {
  db._raw.prepare(
    `INSERT INTO finance_property_monthly (property_key, period, total_revenue_cents) VALUES ('ivanhoe', ?, 100000)`
  ).run(period);
}
...
describe('POST /api/contracts/finance-property-monthly-remove-v1', () => {
  ...
  it('removes a real monthly entry for an admin user, always under the fixed ivanhoe property key', async () => {
    const db = makeTestDb();
    insertMonthly(db, '2027-01');
