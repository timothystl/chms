...
const PATH = '/api/contracts/finance-church-report-trend-v1';
...
describe('handleContractsServiceApi finance-church-report-trend-v1', () => {
  ...
  it('takes no query parameters -- returns a real, valid, empty-years contract when nothing is on file', async () => {
    const res = await call();
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.contract).toBe('connect.finance-church-report-trend.v1');
    expect(body.years).toEqual([]);
    expect(body.reconciliation).toEqual({ yearCount: 0, totalsMatch: true });
  });

  it('reflects real multi-year church-entry rows', async () => {
    const db = makeTestDb();
    db._raw.prepare(
    ...
    db._raw.prepare(
      `INSERT INTO finance_church_entries (fiscal_year, classification, category_path, account_name, own_actual_cents, own_budget_cents, source)
       VALUES (?,?,?,?,?,?,?)`
    ).run(2026, 'Expenses', 'Expenses:50000 Payroll', '50000 Payroll', 900000, 950000, 'import');
    const res = await call({ db });
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.years).toHaveLength(1);
    expect(body.years[0]).toMatchObject({
      fiscalYear: 2026, incomeActualCents: 1300000, expenseActualCents: 900000, netIncomeActualCents: 400000, accountCount: 2,
    });
  });
