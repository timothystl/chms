...
const PATH = '/api/contracts/finance-property-monthly-write-v1';
...
const ROW = {
  period: '2027-01', occupancy_pct: 92.5, total_revenue: '10000', total_expenses: '4000',
...
describe('POST /api/contracts/finance-property-monthly-write-v1', () => {
  ...
  it('saves a real month for an admin user, always under the fixed ivanhoe property key', async () => {
    const db = makeTestDb();
    ...
    const row = db._raw.prepare('SELECT * FROM finance_property_monthly WHERE property_key=? AND period=?').get('ivanhoe', '2027-01');
    expect(row.total_revenue_cents).toBe(1000000);
    expect(row.total_expenses_cents).toBe(400000);
    expect(row.loan_payment_cents).toBe(120000);
    expect(row.interest_expense_cents).toBe(40000);
  });
  ...
  it('re-saving the same period upserts rather than duplicating, same as the legacy route', async () => {
    ...
    await post({ env: baseEnv(db), token, body: { ...ROW, total_revenue: '20000' } });
    const rows = db._raw.prepare('SELECT * FROM finance_property_monthly').all();
    expect(rows).toHaveLength(1);
    expect(rows[0].total_revenue_cents).toBe(2000000);
  });
  ...
  it('still applies the same validation as the human route (bad period -> 400, saving nothing)', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
