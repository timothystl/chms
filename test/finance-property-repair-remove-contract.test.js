...
const PATH = '/api/contracts/finance-property-repair-remove-v1';
...
function insertRepair(db) {
  const r = db._raw.prepare(
    `INSERT INTO finance_property_repairs (property_key, entry_date, category, description, amount_cents, payee) VALUES ('ivanhoe', '2027-01-15', 'HVAC', 'Furnace repair', 45000, 'Acme HVAC')`
  ).run();
...
describe('POST /api/contracts/finance-property-repair-remove-v1', () => {
  ...
  it('removes a real repairs entry for an admin user, always under the fixed ivanhoe property key', async () => {
    const db = makeTestDb();
    const id = insertRepair(db);
    ...
    const row = db._raw.prepare('SELECT * FROM finance_property_repairs WHERE property_key=? AND id=?').get('ivanhoe', id);
  ...
  it('ignores a client-supplied property_key -- the property is always ivanhoe on Connect\'s side', async () => {
    ...
    expect(res.status).toBe(200);
    expect(db._raw.prepare('SELECT * FROM finance_property_repairs').all()).toHaveLength(0);
  });
  ...
  it('rejects a non-numeric id with a 400 and leaves other rows untouched', async () => {
    ...
    expect(db._raw.prepare('SELECT * FROM finance_property_repairs').all()).toHaveLength(1);
  });
