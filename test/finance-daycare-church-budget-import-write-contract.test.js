...
const PATH = '/api/contracts/finance-daycare-church-budget-import-write-v1';
...
describe('POST /api/contracts/finance-daycare-church-budget-import-write-v1', () => {
  ...
  function seedChurchBudget(db, year = 2025) {
    insertChurchRow(db, {
      fiscal_year: year, period_month: 0, classification: 'Expenses',
      category_path: 'Expenses:57 MDO Expenses:57010 MDO Payroll', account_name: 'MDO Payroll',
      depth: 2, has_children: 0, own_actual_cents: 1200000, own_budget_cents: 1100000, source: 'qbo_sync',
    });
  }
  ...
  it('re-derives Daycare entries for a finance-role user (edit on the finance item by default)', async () => {
    ...
    expect(out).toMatchObject({ ok: true, year: 2025, imported: 2, savedBy: 'sarah' });

    const rows = db._raw.prepare(`SELECT * FROM finance_daycare_entries WHERE source='church_budget_import'`).all();
    expect(rows).toHaveLength(2);

    const audit = db._raw.prepare(`SELECT action, entity_type, new_value FROM audit_log WHERE action='daycare_church_budget_import_via_finance'`).get();
