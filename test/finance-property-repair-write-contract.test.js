...
const PATH = '/api/contracts/finance-property-repair-write-v1';
...
const ROW = { entry_date: '2027-01-15', category: 'HVAC', description: 'Replace rooftop unit', amount: '4500', payee: 'Acme HVAC', capitalized: false };

describe('POST /api/contracts/finance-property-repair-write-v1', () => {
  ...
  it('saves a real repair entry for an admin user, always under the fixed ivanhoe property key', async () => {
    const db = makeTestDb();
    ...
    const row = db._raw.prepare('SELECT * FROM finance_property_repairs WHERE id=?').get(out.id);
    expect(row.property_key).toBe('ivanhoe');
    expect(row.category).toBe('HVAC');
    expect(row.amount_cents).toBe(450000);
    expect(row.capitalized).toBe(0);
  });

  it('stores capitalized as 1 when the checkbox is set', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { ...ROW, capitalized: true } });
    const out = await res.json();
    const row = db._raw.prepare('SELECT capitalized FROM finance_property_repairs WHERE id=?').get(out.id);
    expect(row.capitalized).toBe(1);
  });
