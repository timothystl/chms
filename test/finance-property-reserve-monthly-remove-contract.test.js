...
const PATH = '/api/contracts/finance-property-reserve-monthly-remove-v1';
...
function insertReserveMonth(db, reportMonth) {
  db._raw.prepare(
    `INSERT INTO finance_property_reserves (property_key, reserve_key, report_month, reserve_before_cents, contribution_cents, reserve_after_cents)
     VALUES ('ivanhoe', 'property_tax', ?, 0, 100000, 100000)`
  ).run(reportMonth);
}
...
const BODY = { reserve_key: 'property_tax', report_month: '2027-01' };

describe('POST /api/contracts/finance-property-reserve-monthly-remove-v1', () => {
  ...
  it('removes a real reserve schedule month for an admin user, always under the fixed ivanhoe property key', async () => {
    const db = makeTestDb();
    insertReserveMonth(db, '2027-01');
  ...
  it('removing a nonexistent reserve key/report month succeeds as a no-op, same as the legacy DELETE route', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { reserve_key: 'property_tax', report_month: '2099-12' } });
    expect(res.status).toBe(200);
  ...
  it('rejects a bad reserve key, the same shape the legacy route\'s own URL regex enforces', async () => {
    ...
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { reserve_key: 'Bad Key!', report_month: '2027-01' } });
    expect(res.status).toBe(400);
    expect(db._raw.prepare('SELECT * FROM finance_property_reserves').all()).toHaveLength(1);
  });

  it('rejects a bad report_month format with a 400 and leaves other rows untouched', async () => {
    const db = makeTestDb();
    insertReserveMonth(db, '2027-01');
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { reserve_key: 'property_tax', report_month: '2027' } });
    expect(res.status).toBe(400);
    expect(db._raw.prepare('SELECT * FROM finance_property_reserves').all()).toHaveLength(1);
  });
