...
const PATH = '/api/contracts/finance-base-projection-write-v1';
...
// ── Minimal RSA JWT helpers, mirroring test/finance-cash-policy-write-contract.test.js ──
...
describe('POST /api/contracts/finance-base-projection-write-v1', () => {
  ...
  it('saves a whole-dollar Projected correction for an admin, into the real shared store', async () => {
    const db = makeTestDb();
    ...
    const res = await post({
      ...
      body: { year: 2026, rows: [{ category: 'Expenses:Utilities', amount: 1234 }] },
    });
    expect(res.status).toBe(200);
    const out = await res.json();
    expect(out).toEqual({ ok: true, year: 2026, saved: 1, savedBy: 'root' });

    const row = db._raw.prepare(`SELECT value FROM finance_settings WHERE key='finance_base_proj_overrides'`).get();
    expect(JSON.parse(row.value)).toEqual({ '2026': { 'Expenses:Utilities': 123400 } });
  });

  it('clears a category override when the sent amount is blank', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    db._raw.prepare(`INSERT INTO finance_settings (key,value) VALUES ('finance_base_proj_overrides', ?)`)
      .run(JSON.stringify({ '2026': { 'Expenses:Utilities': 123400 } }));
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));

    const res = await post({
      env: baseEnv(db), token,
      body: { year: 2026, rows: [{ category: 'Expenses:Utilities', amount: '' }] },
    });
    const out = await res.json();
    expect(out.ok).toBe(true);
    const row = db._raw.prepare(`SELECT value FROM finance_settings WHERE key='finance_base_proj_overrides'`).get();
    expect(JSON.parse(row.value)).toEqual({ '2026': {} });
  });

  it('does not clobber a different fiscal year already stored', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    db._raw.prepare(`INSERT INTO finance_settings (key,value) VALUES ('finance_base_proj_overrides', ?)`)
      .run(JSON.stringify({ '2025': { 'Expenses:Utilities': 90000 } }));
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));

    await post({ env: baseEnv(db), token, body: { year: 2026, rows: [{ category: 'Expenses:Utilities', amount: 1000 }] } });
    const row = db._raw.prepare(`SELECT value FROM finance_settings WHERE key='finance_base_proj_overrides'`).get();
    expect(JSON.parse(row.value)).toEqual({
      '2025': { 'Expenses:Utilities': 90000 },
      '2026': { 'Expenses:Utilities': 100000 },
    });
  });

  it('rejects a missing year, the same way the legacy route would', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { rows: [{ category: 'x', amount: 1 }] } });
    expect(res.status).toBe(400);
  });

  it('rejects a council user -- base-projection editing is admin-only, like the rest of the Planning table', async () => {
    const db = makeTestDb();
    ...
    const res = await post({ env: baseEnv(db), token, body: { year: 2026, rows: [] } });
  ...
  it('rejects when the shared X-Contract-Key is wrong, before ever looking at identity', async () => {
    ...
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('admin@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, contractKey: 'wrong-secret', body: { year: 2026, rows: [] } });
  ...
  it('rejects a missing or invalid Access assertion even with a correct contract key', async () => {
    const db = makeTestDb();
    insertUser(db, { username: 'root', email: 'admin@timothystl.org', role: 'admin' });
    const noToken = await post({ env: baseEnv(db), body: { year: 2026, rows: [] } });
    expect(noToken.status).toBe(401);
    const garbage = await post({ env: baseEnv(db), token: 'not-a-jwt', body: { year: 2026, rows: [] } });
  ...
  it('returns 503 when Connect has not been configured with the Access team/audience yet', async () => {
    ...
    const res = await post({ env, token, body: { year: 2026, rows: [] } });
