...
const PATH = '/api/contracts/finance-daycare-entry-v1';
...
// ── Minimal RSA JWT helpers, mirroring test/giving-quick-entry-contract.test.js ──
...
const ROW = { period: '2027', category: 'Tuition income', entry_type: 'actual', amount_cents: 500000 };

describe('POST /api/contracts/finance-daycare-entry-v1', () => {
  ...
  it('records a real entry for a finance-role user (edit on the finance item by default)', async () => {
    const db = makeTestDb();
    ...
    const row = db._raw.prepare('SELECT period, category, entry_type, amount_cents FROM finance_daycare_entries WHERE id=?').get(out.id);
    expect(row).toEqual(ROW);

    const audit = db._raw.prepare(`SELECT action, entity_type, new_value FROM audit_log WHERE action='daycare_entry_via_finance'`).get();
    expect(audit).toEqual({ action: 'daycare_entry_via_finance', entity_type: 'finance_daycare_entries', new_value: 'sarah@timothystl.org' });
  });

  it('allows council by default, since compensation defaults to edit and finance/daycare accepts any of finance/budget/compensation', async () => {
    const db = makeTestDb();
  ...
  it('rejects a staff-role user -- finance/budget/compensation all default to none for staff', async () => {
    ...
    expect(db._raw.prepare('SELECT * FROM finance_daycare_entries').all()).toHaveLength(0);
  });
  ...
  it('still applies the same validation as the human route (missing category -> 400, saving nothing)', async () => {
    ...
    insertUser(db, { username: 'sarah', email: 'sarah@timothystl.org', role: 'finance' });
    const token = await signToken(keyPair.privateKey, kid, accessPayload('sarah@timothystl.org'));
    const res = await post({ env: baseEnv(db), token, body: { period: '2027', entry_type: 'actual', amount_cents: 500000 } });
