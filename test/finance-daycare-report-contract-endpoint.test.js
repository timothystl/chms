...
const SEG = 'contracts/finance-daycare-report-v1';
...
describe('contracts/finance-daycare-report-v1 access gate', () => {
...
describe('contracts/finance-daycare-report-v1 real handler', () => {
  ...
  it('returns a real 200 empty-categories contract for a fiscal year with nothing on file', async () => {
    const env = { DB: makeTestDb() };
    ...
    expect(body.contract).toBe('connect.finance-daycare-report.v1');
    expect(body.fiscalYear).toBe(2030);
    expect(body.categories).toEqual([]);
  });

  it('reflects real daycare-entry rows for the requested fiscal year only, excluding non-counted sources', async () => {
    const db = makeTestDb();
    db._raw.prepare(
      `INSERT INTO finance_daycare_entries (period, category, entry_type, amount_cents, source) VALUES (?,?,?,?,?)`
    ).run('2026', 'Tuition Income', 'actual', 40000000, 'church_budget_import');
    db._raw.prepare(
      `INSERT INTO finance_daycare_entries (period, category, entry_type, amount_cents, source) VALUES (?,?,?,?,?)`
    ).run('2026', 'Tuition Income', 'actual', 999999999, 'daycare_api');
    db._raw.prepare(
      `INSERT INTO finance_daycare_entries (period, category, entry_type, amount_cents, source) VALUES (?,?,?,?,?)`
    ).run('2027', 'Tuition Income', 'actual', 999999999, 'church_budget_import');
    const env = { DB: db };
    ...
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.categories).toEqual([{ category: 'Tuition Income', classification: 'Income', actualCents: 40000000, budgetCents: 0 }]);
  });
});
