...
function postForm(env, { accessJwt, body } = {}) {
  const params = new URLSearchParams({
    rows: '2023,Tuition Income,actual,142000,\n2023,Payroll,actual,95000,',
    ...body,
  });
  return worker.fetch(new Request('https://finance.test/api/v1/connect-daycare-bulk-write', {
    method: 'POST',
    headers: {
...
describe('Daycare Report — bulk-paste form and relay route', () => {
  it('rejects the wrong method on the write-only route with an Allow header naming only POST', async () => {
    const res = await worker.fetch(new Request('https://finance.test/api/v1/connect-daycare-bulk-write'), baseEnv);
    expect(res.status).toBe(405);
    expect(res.headers.get('allow')).toBe('POST');
  });

  it('does not show the bulk form when the viewer role is not verified (fail-closed on the write UI)', async () => {
    const res = await worker.fetch(new Request('https://finance.test/?section=daycare&page=actuals'), baseEnv);
    expect(res.status).toBe(200);
    const html = await res.text();
    expect(html).not.toContain('/api/v1/connect-daycare-bulk-write');
  });

  it('shows the bulk form for any verified role that can reach the Daycare section, e.g. finance', async () => {
    ...
    const html = await res.text();
    expect(html).toContain('<form method="POST" action="/api/v1/connect-daycare-bulk-write">');
    expect(html).toContain('name="rows"');
  });
  ...
  it('forwards the Access assertion and parses pasted rows into cents, redirecting to status=ok on success', async () => {
    let captured;
    ...
    expect(url.pathname).toBe('/api/contracts/finance-daycare-bulk-write-v1');
    const sentBody = JSON.parse(await captured.text());
    expect(sentBody).toEqual({ rows: [
      { period: '2023', category: 'Tuition Income', entry_type: 'actual', amount_cents: 14200000, notes: '' },
      { period: '2023', category: 'Payroll', entry_type: 'actual', amount_cents: 9500000, notes: '' },
    ] });
    ...
    expect(html).toContain('Rows recorded in Connect.');
  });
