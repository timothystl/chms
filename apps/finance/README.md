...
- `data-pages.js`, `quickbooks-snapshot-service.js`, `finance-data-imports-client.js` — Accounts & Data → Data & Imports, organized like Connect's legacy tab: connections (QuickBooks, daycare app, board packet JSON download at `/api/v1/board-packet-export`), every importer with its last run (`finance-import-status-v1`) and a link to the Finance page that runs it, the MDO-from-Church-Budget preview (`finance-daycare-church-budget-preview-v1`) before the existing import relay (also on Daycare Report → Actuals detail), hand-entered adjustments and removal controls linked to their pages, classification and policy, and the raw QuickBooks Budget vs. Actual and account balances read from Finance's own `finance_qb_snapshot` cache (never a QuickBooks call). The three read contracts live in `src/api-data-imports-contracts.js` and reuse the legacy route functions; import status and the preview are answered from Finance's database, the board packet from Connect because it includes Giving fund totals. Tests: `test/finance-data-imports.test.js`.
...
- `health-parity-pages.js`, `finance-health-client.js` — Full detail also shows everything Connect's legacy Financial Health tab shows (revenue mix and control band, stream cards, designated funds and their tie-out, the money-flow Sankey or Share donuts, the three engines, General Fund giving pace, cash runway against the policy floor, five years of the mix, the fundraising callout, the appeal ladder, the three levers and the computed decisions), from `connect.finance-health.v1` (`src/api-finance-health-contract.js`). Connect builds it with the legacy tab's own server functions (`readChurchThisYear`, `buildChurchMultiYear`, `readDaycareAllocation`, `readDaycareRooms`, `readPropertyPayload`) and ports the tab's few browser-side derivations, so Finance renders finished figures. It reads Giving rollups, so Connect always answers it (it is not a local read). Charts are inline SVG or CSS; the Flow/Share and Gap/Gap + reserves toggles are links (`flow=share`, `appeal=gap`). When it is live, the entity views use its periods (this fiscal year's church ledger, this calendar year's daycare with the MDO share of utilities and insurance); when it fails, the page keeps its other panels and says the Connect figures could not be read.
...
- `role-cache.js`, `access-jwt.js` — Page views keep working when Connect cannot confirm a role. Each successful `staff-role-v1` answer is saved in Finance's `finance_role_cache` table (migration `0015_finance_role_cache.sql`, created on first use). When Connect times out, drops the connection or returns a 5xx, Finance verifies the Cloudflare Access sign-in itself (`FINANCE_ACCESS_TEAM_DOMAIN`/`FINANCE_ACCESS_AUD`, the same values Connect uses) and applies the role saved for that person within the last seven days, with a notice on the page. A 401/403 refusal from Connect is never overridden, and every save still requires a live check. Roles are still managed only in Connect.
- `quickbooks-transactions-service.js`, `quickbooks-pages.js` — QuickBooks Transactions, Expense drill-down, Vendor spend, and Exceptions are live, read-only report views over Finance's own QuickBooks connection. The selected date range is validated and limited to one year per request; Finance does not retain transaction detail. Expense/vendor totals use only expense-like QuickBooks transaction types, exceptions are limited to observable missing/invalid fields, and supported rows link back to QuickBooks for any edit.
- `import-history-service.js`, migration `0016_finance_import_history.sql` — Import history is append-only for newly completed imports while `finance_import_log` remains the latest-status index. On first view, each importer's retained latest status is copied in as a labeled baseline, preserving useful pre-history information without inventing older events.
- `property-books-service.js`, `property-books-pages.js` — Commercial Property's Receivables & deposits and Position & bank rec pages. Tenant aging and security deposits (entered or pasted from the manager's monthly reports; pasting a month replaces that month) and monthly reconciliations of the property's own bank account are stored in Finance's `finance_property_receivables`/`finance_property_bank_recs` tables (migration `0014_finance_property_books.sql`, created on first use). The position adds rent owed and subtracts deposits and reserves from the reconciled cash. Edits are admin-only. The service also supplies the payoff-by-year and extra-principal math the Debt payoff page draws from `finance-property-debt-v1`. `property-acquisition-pages.js` is the Acquisition model, a GET-form what-if that saves nothing.
...
- `board-layout.js`, `finance-board-layout-client.js` — the legacy Budget Planner's Board view layout (September 26). `finance-board-layout-v1` (`src/api-board-layout-contracts.js`) returns the saved board categories, heading and account renames, Donor Income wrapper title and purpose tags. The Budget builder groups its lines by board category in legacy's order, with a heading and subtotal for each category, Unrestricted and Restricted gifts under the Donor Income wrapper, and a QuickBooks-order toggle (`view=qb`). Accounts with no saved category use legacy's name-based default. On Chart of Accounts, an admin gets the Budget layout editor: category heading renames, and one table of every leaf account with a display name, a board category picker, a purpose tag picker and a bulk move. Only changed rows are sent through the existing `connect-board-categories-write` and `connect-purpose-tags-write` relays, so an account on its automatic category is not pinned to it. The tag-list form now lists every saved tag, including unused ones.
- `accounts-pages.js`, `accounts-report-service.js`, `finance-chart-of-accounts-client.js` — Accounts & Data → Chart of accounts, matched to Connect's legacy Chart of Accounts tab (September 26). `connect.finance-chart-of-accounts.v1` is now year-scoped: `?fiscal_year=` (default: the church's current calendar year, with no fallback to an older year, as legacy) returns that year's accounts in every P&L classification, picked with the same source precedence as the Church Report, each with its own actual, budget and display name; the consumer still accepts the older year-less shape. The page has a fiscal-year selector (a GET form), legacy's leaves (lines with no actual and no budget left out) grouped by board category in legacy's order with renames, actuals, budgets and subtotals, and Resources by Purpose. Payroll cost appears there only for roles that can read the Compensation plan (the plan year after the chart's year, as legacy); otherwise the report counts tagged accounts only and says so. A renamed account is matched to a tagged worker by its QuickBooks number, where legacy used the display name.
- `connect-planner.js` — Compensation → Connect planner (September 26, for side-by-side comparison): Connect's own Salary Planner (1 · Set pay, 2 · Check fairness, 3 · Health plan, This year's rates, Council summary) running unchanged inside Finance. `/connect-planner` serves Connect's front-end scripts and stylesheet from `src/html-chms.js` with its own CSP (the only Finance page that runs script, framed only by Finance). The planner's `api()` calls are pointed at `/api/v1/connect-planner/*`, which relay to `finance-compensation-plan-v1`, `finance-compensation-write-v1`, `connect.finance-church-report.v1` and `finance-board-layout-v1`, re-checking the viewer's Connect role on every call. A council member's changes are saved as their private draft in Finance's database, never the shared plan. This adds about 0.8 MB (compressed) to the Finance Worker; remove the route, page and module when the comparison is done.
- `compensation-plan-form.js`, `compensation-settings-pages.js` — the rest of the legacy Salary Planner's editing surface for admin/compensation (September 26), saved through the same `compensation-plan-write-v1` fetch-edit-resubmit relay as the roster editor (`action` = `methods`, `reference`, `quote` or `ranges`):
  - Plan: raise methods, with the plan-wide method, a per-worker method, and a hand-set salary for each worker beside every method's figure. The page also sets the custom % and share of scale, the base-year comparison basis, and an apply-to-everyone option.
  - Worker form: FTE, cash only, externally funded, coverage tier, and the hand-set opt-out and employee-only premium.
  - Compensation → Rates & ranges: each year's district base salary, Concordia rates, Social Security COLA, opt-out cash and source documents. It also holds the health plan quote per option and tier, the covered plan and family size, and each worker's Concordia ranges. The ranges can also be edited from Benchmarks.
  Council and other roles see Rates & ranges read-only. Stored shapes match legacy: fractions for percentages, cents for money, and per-worker maps by roster index.
...
## QuickBooks OAuth/sync design

Superseded status: Finance owns the live QuickBooks connection as of September 25, 2026 (`FINANCE_QB_ENABLED=1`, Connect's `QBO_MANAGED_BY_FINANCE=1`); see `docs/QUICKBOOKS_FINANCE_CUTOVER.md` and `quickbooks-oauth-routes.js`. The design notes below are historical.
