...
- `property-books-service.js`, `property-books-pages.js`, `finance-property-loan-client.js` — Commercial Property's Receivables & deposits, Position & bank rec, and Debt payoff pages. Tenant aging and security deposits (entered or pasted from the manager's monthly reports; pasting a month replaces that month) and monthly reconciliations of the property's own bank account are stored in Finance's `finance_property_receivables`/`finance_property_bank_recs` tables (migration `0014_finance_property_books.sql`, created on first use). The debt page reads Connect's loan record and each month's reported payment and interest through `finance-property-loan-v1` (`src/api-property-loan-contracts.js`), rolls the last confirmed balance forward with Connect's own rule, and projects the payoff; a recorded loan statement is merged into Connect's loan record through the existing property meta-write relay. All edits are admin-only. `property-acquisition-pages.js` is the Acquisition model, a GET-form what-if that saves nothing.
