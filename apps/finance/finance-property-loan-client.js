import { acceptFinancePropertyLoanV1 } from '../../contracts/validators/finance-property-loan-consumer.js';

const REQUEST_TIMEOUT_MS = 4000;

export async function fetchFinancePropertyLoan(env, propertyKey = 'ivanhoe') {
  if (!env.CONNECT_SERVICE || !env.FINANCE_CONTRACT_API_KEY) return { ok: false, reason: 'not_configured' };
  const url = `https://connect.timothystl.org/api/contracts/finance-property-loan-v1?property_key=${encodeURIComponent(propertyKey)}`;
  let response;
  try {
  ...
  try { return { ok: true, loan: acceptFinancePropertyLoanV1(payload) }; }
