import { acceptFinanceClassificationV1 } from '../../contracts/validators/finance-classification-consumer.js';

const REQUEST_TIMEOUT_MS = 4000;

export async function fetchFinanceClassification(env, fiscalYear) {
  ...
  try {
    response = await binding.fetch(new Request(
      `https://connect.timothystl.org/api/contracts/finance-classification-v1?fiscal_year=${encodeURIComponent(fiscalYear)}`,
  ...
  try { return { ok: true, classification: acceptFinanceClassificationV1(payload) }; }
