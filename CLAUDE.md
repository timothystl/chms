...
- [ ] **FIN2** — Live QuickBooks verification in progress (sandbox company, 2026-07-16): connecting immediately hit a real bug — the OAuth callback always failed `{"error":"Unauthorized"}` because the `vol_auth` cookie was `SameSite=Strict`, which drops the cookie on Intuit's cross-site redirect back to our callback URL. Fixed in v1.24.1 (changed to `SameSite=Lax`) — see NOTES.md. Still needs: re-test connect → sync → disconnect → reconnect end-to-end against the sandbox company now that the cookie fix is live, then repeat against the real Production company/keys.
