...
- [x] **SC7** — Scheduler print view redesign, implemented from a design handoff bundle. Clicking Print (More ▸ Print) no longer calls `window.print()` directly — it opens a print-preview overlay with a 3-way mode switch: **Single Sunday** (one roster page, portrait letter, big centered date/ordinal header + per-service role rows + a Both Services section), **Full Month** (a 9-column color-coded table, landscape letter, replaces the old default print output), and **Bulletin Insert** (a black-and-white 7in×8.5in half-sheet for the coming Sunday only, sized for this church's physical bulletin fold). A Sunday `<select>` (Single Sunday / Bulletin Insert only) defaults to whichever Sunday is selected in Focus Week. Per-mode `@page` sizing is applied by rewriting a dedicated `<style>` element's content in JS rather than CSS custom properties (`@page` does not reliably accept `var()` for `size`). Hiding everything else during print is done imperatively via `beforeprint`/`afterprint` (not a `body.pp-printing` CSS class) because in embedded mode (`scheduler-inline.js`) the script's `document.body` is the whole ChMS page, not the `.sched-root` wrapper, so a body-scoped selector would never match there; a CSS class also wouldn't survive `window.print()` being non-synchronous in some browsers (Safari). The old grid-based `@media print` rules are left in place untouched as a harmless fallback for a raw Ctrl+P outside this flow. Verified with a Node harness that extracted and evaluated the actual served (built) script — not a reimplementation — covering XSS-escaping of volunteer names, unfilled-role rendering (blank for Single Sunday/Bulletin, em dash for Full Month, matching the design spec exactly), label-pill display, and the Preacher/Children's Message split for the month table. `npm test` (103/103), `node --check` on both the standalone and embedded built `<script>` blocks, `scheduler/index.html` resynced via the established SC5 process (evaluate-and-write-verbatim, not copy source). **Not verified**: an actual browser/print dialog. Done 2026-07-20 (v1.40.0). (`src/scheduler-html.js`, `scheduler/index.html`)
