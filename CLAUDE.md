...
- [ ] **SC5** — `scheduler/index.html` needs a full resync with `src/scheduler-html.js` again — as of SC3-BUG2 it's missing several newer email-template functions entirely (open-slot notification, weekly reminder), not just behind on the syntax fix. Low urgency since `scheduler/index.html` is a design-reference copy only and isn't served, but letting the drift continue makes it useless as a reference. (noted 2026-07-10)
