...
- [ ] **RDS2** — People tab: add the master-detail quick-view panel (household-grouped list + right-side preview showing contact info, household member chips, Call/Full Profile actions).
...
- [ ] **RDS4** — Giving: add the Batches/Transactions view toggle + fund/date-range filters (needs its own scoping — bigger than pure reskin, see decision above) + stat-tile row; Reports: card/token alignment (already has a customizable-tile structure close to the mockup).
