...
**Still deliberately deferred**: charts for the new YoY table (a shared `renderGroupedBarChart()` helper needs extracting from Attendance's existing chart first — currently a plain table), CSV/Excel import (the real backfill/resilience mechanism — QuickBooks Budget-vs-Actuals export parsing, a bounded-lookahead hierarchy-detection algorithm already designed since a naive stack parser is provably insufficient), and the Finance nav consolidation (merging Giving/Tuition Aid/Finance into one sidebar entry with a flat sub-nav — mechanically scoped already, no dependency on the rest of this work, so it can happen any time). **Not verified**: an actual live QuickBooks sync (still blocked by the 5020 Permission Denied error, FIN2) or a real browser.
