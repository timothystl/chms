...
- [ ] **RDS2b (deferred, not scoped)** — Grouping the People list itself by household (as the mockup shows) was intentionally not done in RDS2. The list is server-side paginated (25/page, sortable) — regrouping by household would need either a new paginated-by-household-group endpoint or client-side re-sorting that risks splitting a household across pages. The mockup's own README says to preserve current behavior/data bindings exactly, so this was treated as a bigger structural change than "add a quick-view panel" and left out rather than guessed at. Revisit only if there's a real ask for it — the quick-view panel already surfaces the household context (link + member chips) without the list needing to be regrouped.
- [ ] **RDS3** — Households + Person Profile: apply the borderless soft-shadow card treatment (already structurally close to the mockup — mostly token/spacing work, not restructuring).
...
- [ ] **RDS4b** — Reports: card/token alignment (already has a customizable-tile structure close to the mockup).
- [ ] **RDS5** — Scheduler/Volunteers alignment to the same card system (Volunteers already has an `ev-*` themed master-detail from VUX9-16 — mostly a token-consolidation exercise per PAL2/PAL4 rather than new structure; Scheduler is a separate embedded shell, lower priority per the mockup's own "will follow the same shell" note).
