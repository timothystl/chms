...
**Nav consolidation — shipped but PAUSED, needs design input, do not resume without it.** Collapsed the Giving/Tuition Aid/Finance sidebar entries into one "Finance" item with a shared flat sub-nav bar across all three tab-panels, and physically moved the 8 giving-related report tiles out of the Reports tab into a new "Giving Reports" section under Finance. This shipped (merged to `main`, confirmed present in the served source), but the user reports it's **not displaying correctly live — confirmed not a caching issue** (checked in incognito, still wrong). No live browser was available to verify this change visually before shipping, and that gap is exactly what bit us: the root cause of the display problem hasn't been diagnosed yet. **User's explicit direction**: stop iterating on this blind — leave it as a backlog item, and loop in design before touching it again, specifically covering: the sidebar/sub-nav structure itself, AND a broader look at "moving of reports, and organizing that better" (i.e. this isn't just a bug to squash — the whole reports-relocation approach may need to be rethought with design, not just patched). Next session on this should start with an actual browser/screenshot to see what's actually wrong before writing any more code.

**Still deliberately deferred** (independent of the above): CSV/Excel import (the real backfill/resilience mechanism — QuickBooks Budget-vs-Actuals export parsing, a bounded-lookahead hierarchy-detection algorithm already designed since a naive stack parser is provably insufficient — no real export file available yet to validate a parser against). **Not verified**: an actual live QuickBooks sync (still blocked by the 5020 Permission Denied error, FIN2) or a real browser (see nav-consolidation note above — this is the recurring gap).
