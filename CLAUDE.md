...
- [x] **FIN5** — Board-level reporting: Finance tab split into Overview/Church Report/Daycare Report sub-tabs, both year-by-year (calendar year), on-screen + printable/exportable. Daycare Report aggregates the flat sync rows client-side (Actual/Budget by category and year, Income/Expense/Net summary rows). Church Report adds a new QBO multi-year Profit & Loss sync (`quickbooks.js` `profitAndLoss()`, cached as `profit_and_loss_by_year`) with 3 daycare tie-in lines (Tuition Income/Payroll wages/Total Expenses) shown for reference alongside QuickBooks' own totals, not merged into them. Done 2026-07-17 (v1.26.0). See NOTES.md for full detail. **Not verified**: real QuickBooks P&L data (no live connection this session) or an actual browser — next "Sync Now" click populates the new cache key for the first time.
