...
- [ ] **PAL1** — Define the canonical extended Palette A token set: derive semantic tokens + shades/tints needed to fully replace palettes B (legacy Steel: `--steel-anchor`, `--mid-steel`, `--sage`, etc.), C (`--ev-*` Events/Volunteers), and D (separate public-site tokens in `src/public/head.js`) from the 4 core brand colors (navy `#1E2D4A`, teal `#2E7EA6`, gold `#C9973A`, cream `#F8F4EE`).
...
- [ ] **PAL3** — Consolidate the public volunteer site (`src/public/head.js` and all `src/public/**/*.js`) onto the same Palette A token set.
- [ ] **PAL4** — Consolidate the scheduler's own `:root` token block in `src/scheduler-html.js` (currently its pre-rebrand "Steel & Amber" palette, `--steel-anchor:#0A3C5C`/`--amber:#D4922A`/Lora+Source Sans 3) onto Palette A — now lower-risk since the standalone route is retired and only the embedded (already-overridden) view is user-facing, but the source tokens should still match for consistency and to remove the now-pointless override-on-embed indirection.
- [ ] **PAL5** — Sweep and eliminate the ~171 hardcoded inline hex colors identified in the audit (138 admin-app occurrences, mostly in `js-reports.js`/`js-attendance.js` chart code and inline `style="..."` strings across `js-giving.js`/`js-reports.js`/`js-attendance.js`/`js-settings.js`/most of `js-people.js`; 33 public-site occurrences), converting to references against the consolidated token set — classes where practical, CSS custom properties where a class isn't a good fit (e.g. dynamically-generated chart SVG colors).

**Scope note:** This is the largest remaining piece of pre-redesign work — it touches nearly every frontend file in the app. Expect to do it in several batches (e.g. one per PAL2/PAL3/PAL4 file group), verifying `node --check` + `npx vitest run` after each batch, same as every other batch this session.
