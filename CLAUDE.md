...
- [ ] **AT5** — Christmas Day/Eve attendance not shown in the chart. The `/reports/attendance-by-time` query filters `service_type='sunday'` for the per-date list (api-chms.js line 1904), and the yearly summary queries also filter `service_type='sunday'` (lines 1869, 1879). Mid-week Christmas services (stored with service_type != 'sunday', or stored as 'sunday' on a non-Sunday date) are therefore hidden. Need to decide: (a) include all service_type values in the chart, (b) add a "special services" toggle, or (c) audit data to confirm Christmas services are actually being stored and with what service_type. Also the "Add Sunday" button / "Pre-fill Year Sundays" only generate Sunday entries, so Christmas may need a separate "Add Special Service" flow. (noted 2026-04-22)
...
- [~] **FU1** — Prayer request tracking. **Intake + schema done 2026-04-23 (v107)** via `POST /api/intake/prayer` (see WC1). New `prayer_requests` table stores person_id (nullable for anonymous), submitter_name/email/phone, request_text, is_urgent, share_publicly, status (`new|praying|answered|closed`), resolution_note, source. Remaining: admin UI (list view, filters, status workflow, resolution notes) and eventual surfacing on the dashboard's weekly workflow card.
- [x] **FU2** — New-contact follow-up tracking. Done 2026-04-22 (v104). Auto-captures `first_contact_date = today` on manual person create (Breeze imports stay blank). New `GET /admin/api/engagement/followup-queue` returns non-member active records with first_contact_date set and followup_status != 'done', newest first. `POST /admin/api/engagement/update-followup` updates followup_status / followup_notes / first_contact_date (all optional). Dashboard "New Contacts" card (toggle-able via Customize) shows first 5 with Notes / Done / Open actions; Notes button opens a prompt that saves text + auto-sets status='in_progress'; Done sets status='done' and removes from queue.
...
- [x] **WC1** — Electronic contact card intake. Done 2026-04-23 (v107). Two new server-to-server endpoints on this Worker, authed by `X-Intake-Key` header against the `CHMS_INTAKE_API_KEY` secret: `POST /api/intake/connect-card` (creates or merges a person by email→phone; seeds `first_contact_date` + `followup_status='new'` so the record lands in the FU2 follow-up queue) and `POST /api/intake/prayer` (writes a `prayer_requests` row, see FU1). Every payload is mirrored into `intake_submissions` for forensic replay. Routes are mounted in `tlc-volunteer-worker.js` before the scheduler auth gate and before the `/api/*` Breeze catch-all. Handler: `src/api-intake.js`. The public church website Worker is expected to POST JSON bodies (schemas documented in that file's comments).
...
- [ ] **IN6** — Secrets inventory doc. No single source of truth for which secrets the Worker needs. Add a `SECRETS.md` (or section in CLAUDE.md) listing every env var, what it's for, and rotation steps. Grep-current-list: `ADMIN_PASSWORD`, `RESEND_API_KEY`, `EMAIL_FROM`, `BREVO_API_KEY`, `BREVO_LIST_ID`, `CHMS_INTAKE_API_KEY` (website form intake auth), plus any Breeze creds.
- [ ] **IN7** — D1 schema migrations system. Schema changes are currently ad-hoc `wrangler d1 execute` calls with no version tracking. Add a `migrations/` directory with numbered SQL files and adopt `wrangler d1 migrations apply` (or a minimal home-grown runner) so prod schema state is reproducible and reviewable.
- [ ] **IN8** — Audit log retention / pruning. `audit_log` grows without bound; G9 raised the in-memory limit to 10k rows which is a symptom. Add a TTL prune job (e.g. drop rows older than 180 days, keep delete-audit rows longer). Run from the existing daily cron.
