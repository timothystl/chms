...
- [ ] **PAL3** — Consolidate the public volunteer site (`src/public/head.js` and all `src/public/**/*.js`) onto the same Palette A token set. Note: `src/public/head.js` also contains dead mockup-only CSS (`.annotation-bar`, `.page-divider` — never applied to any real markup, confirmed via grep) worth deleting in the same pass.
...
**Scope note:** PAL2/PAL3/PAL5 are the largest remaining pieces of pre-redesign work — they touch nearly every frontend file in the app and change real rendered output (not just documentation/zero-risk aliasing like PAL1/PAL4), so they need visual verification, not just `node --check`/`npx vitest run`. Expect several more batches.
